...
describe('nighthawkBaseUrl', () => {
  it('strips trailing slashes from the NIGHTHAWK_BASE_URL override', () => {
    ...
    vi.stubEnv('NIGHTHAWK_BASE_URL', 'https://gw.example.com/');
    expect(nighthawkBaseUrl()).toBe('https://gw.example.com');
    expect(nighthawkUsageUrl()).toBe('https://gw.example.com/usages');
  });
});
...
describe('isManagedNighthawkBaseUrl', () => {
  ...
  it('rejects other paths on the managed host and other hosts entirely', () => {
    expect(isManagedNighthawkBaseUrl('https://api.kimi.com/coding/v2')).toBe(false);
    expect(isManagedNighthawkBaseUrl('https://api.kimi.com/v1')).toBe(false);
    expect(isManagedNighthawkBaseUrl('https://gateway.example.com/coding/v1')).toBe(false);
    expect(isManagedNighthawkBaseUrl('https://api.moonshot.cn/v1')).toBe(false);
  });

  it('rejects undefined and unparseable values', () => {
    expect(isManagedNighthawkBaseUrl(undefined)).toBe(false);
    expect(isManagedNighthawkBaseUrl('')).toBe(false);
    expect(isManagedNighthawkBaseUrl('not a url')).toBe(false);
  });
});

describe('isManagedNighthawk', () => {
  it('matches only the nighthawk managed provider', () => {
    expect(isManagedNighthawk('managed:nighthawk')).toBe(true);
    expect(isManagedNighthawk('managed:nighthawk-ai')).toBe(false);
    expect(isManagedNighthawk('openai')).toBe(false);
    expect(isManagedNighthawk('')).toBe(false);
    expect(isManagedNighthawk(null)).toBe(false);
    expect(isManagedNighthawk()).toBe(false);
  });
});
...
describe('fetchManagedUsage', () => {
  ...
  it('falls back to local usage hints when the API error body is empty', async () => {
    ...
    if (result.kind !== 'error') return;
    expect(result.status).toBe(404);
    expect(result.message).toBe('Usage endpoint not available. Try NightHawk For Coding.');
  });
});
