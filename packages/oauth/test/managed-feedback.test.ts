...
const SAMPLE_BODY: SubmitFeedbackBody = {
  ...
  info: { tool: 'nighthawk-cli', env: 'test' },
};
...
describe('nighthawkFeedbackUrl', () => {
  ...
  it('honours NIGHTHAWK_BASE_URL and trims trailing slashes', () => {
    vi.stubEnv('NIGHTHAWK_BASE_URL', 'https://example.test/v9///');
    expect(nighthawkFeedbackUrl()).toBe('https://example.test/v9/feedback');
  });
});
...
describe('fetchSubmitFeedback', () => {
  ...
  it('preserves the nighthawk- version prefix in the request body', async () => {
    const fetchMock = vi.fn(async () =>
      new Response(JSON.stringify({ feedback_id: 3 }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
    ...
    expect(sent['version']).toBe('nighthawk-0.1.1');
  });
