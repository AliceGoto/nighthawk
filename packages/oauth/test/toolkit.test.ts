...
describe('resolveNighthawkTokenStorageName', () => {
  it('maps config oauth keys to the file storage token name', () => {
    expect(
      resolveNighthawkTokenStorageName({
        providerName: NIGHTHAWK_PROVIDER_NAME,
        oauthKey: 'oauth/nighthawk',
      }),
    ).toBe('nighthawk');
    expect(resolveNighthawkTokenStorageName({ oauthKey: 'nighthawk' })).toBe('nighthawk');
  });
  ...
  it('accepts non-managed providers with a valid key and rejects unsafe token keys', () => {
    ...
    expect(
      resolveNighthawkTokenStorageName({
        providerName: 'nighthawk-anthropic',
        oauthKey: 'oauth/nighthawk',
      }),
    ).toBe('nighthawk');
    expect(() => resolveNighthawkTokenStorageName({ oauthKey: '../nighthawk' })).toThrow(/Invalid/);
  });
});

describe('NighthawkOAuthToolkit', () => {
  it('can be constructed without host identity', async () => {
    const storage = new MemoryTokenStorage();
    storage.tokens.set('nighthawk', token('access-1'));
    const toolkit = new NighthawkOAuthToolkit({
      homeDir: join('/tmp', 'nighthawk-oauth-toolkit-test'),
      storage,
      now: () => 100,
    });
  ...
  it('resolves bearer token providers using the configured oauth key', async () => {
    const storage = new MemoryTokenStorage();
    storage.tokens.set('custom-nighthawk', token('custom-access'));
    const toolkit = new NighthawkOAuthToolkit({
      homeDir: join('/tmp', 'nighthawk-oauth-toolkit-test'),
      identity: TEST_IDENTITY,
      storage,
    ...
    await expect(
      toolkit
        .tokenProvider(NIGHTHAWK_PROVIDER_NAME, { key: 'oauth/custom-nighthawk' })
        .getAccessToken(),
  ...
  it('refreshes configured bearer token refs against their OAuth host', async () => {
    ...
    storage.tokens.set(resolveNighthawkTokenStorageName({ oauthKey }), {
    ...
    await expect(
      toolkit
        .tokenProvider(NIGHTHAWK_PROVIDER_NAME, { key: oauthKey, oauthHost })
        .getAccessToken(),
  ...
  it('does not reuse a cached OAuth manager across different hosts for the same token key', async () => {
    const storage = new MemoryTokenStorage();
    storage.tokens.set('custom-nighthawk', {
    ...
    await expect(
      toolkit
        .tokenProvider(NIGHTHAWK_PROVIDER_NAME, {
          key: 'oauth/custom-nighthawk',
  ...
  it('resolves cached access tokens using the configured oauth key', async () => {
    const storage = new MemoryTokenStorage();
    storage.tokens.set('custom-nighthawk', token('custom-cached-access'));
    const toolkit = new NighthawkOAuthToolkit({
      homeDir: join('/tmp', 'nighthawk-oauth-toolkit-test'),
      identity: TEST_IDENTITY,
      storage,
    ...
    await expect(
      toolkit.getCachedAccessToken(NIGHTHAWK_PROVIDER_NAME, { key: 'oauth/custom-nighthawk' }),
  ...
  it('provisions managed config after login when an adapter is configured', async () => {
    ...
    const toolkit = new NighthawkOAuthToolkit({
      ...
      configAdapter: {
        ...
        write,
        apply: (target, input) => {
          target.providers[NIGHTHAWK_PROVIDER_NAME] = {
            type: 'nighthawk',
            apiKey: '',
          };
          return {
            defaultModel: `nighthawk/${input.models[0]?.id ?? 'unknown'}`,
  ...
  it.each([401, 402])(
    ...
    async (status) => {
      ...
      const oauthKey = resolveNighthawkOAuthKey({ oauthHost });
      storage.tokens.set(resolveNighthawkTokenStorageName({ oauthKey }), token('stale-access'));
      const fetchMock = vi
        .fn()
  ...
  it('uses a scoped credential slot for non-default OAuth login environments', async () => {
    const storage = new MemoryTokenStorage();
    storage.tokens.set('nighthawk', token('prod-access'));
    ...
    const devOauthKey = resolveNighthawkOAuthKey({
      oauthHost: devOauthHost,
      baseUrl: devBaseUrl,
    });
    const devStorageName = resolveNighthawkTokenStorageName({ oauthKey: devOauthKey });
    const write = vi.fn();
    ...
    expect(storage.tokens.get('nighthawk')?.accessToken).toBe('prod-access');
    expect(storage.tokens.get(devStorageName)?.accessToken).toBe('dev-access');
    expect(config.providers[NIGHTHAWK_PROVIDER_NAME]?.oauth).toEqual({
      storage: 'file',
      key: devOauthKey,
      oauthHost: devOauthHost,
    });
  ...
  it('removes managed config on logout when an adapter supports cleanup', async () => {
    const storage = new MemoryTokenStorage();
    storage.tokens.set('nighthawk', token('access-1'));
    const config = { providers: { [NIGHTHAWK_PROVIDER_NAME]: { type: 'nighthawk' } } };
    const write = vi.fn();
    const remove = vi.fn();
    const toolkit = new NighthawkOAuthToolkit({
      homeDir: join('/tmp', 'nighthawk-oauth-toolkit-test'),
      ...
      configAdapter: {
        read: () => config,
        write,
        apply: () => ({ defaultModel: 'nighthawk/nighthawk', defaultThinking: true }),
        remove,
      },
    });
    ...
    expect(write).toHaveBeenCalledWith(config);
    await expect(storage.load('nighthawk')).resolves.toBeUndefined();
  });
});
