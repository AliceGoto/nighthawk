 ...
 *
 * Only `managed:nighthawk` is supported today. The platform exposes a
...
export function isManagedNighthawk(providerKey?: string | null): boolean {
  ...
  return providerKey.slice(MANAGED_PREFIX.length) === NIGHTHAWK_PLATFORM_ID;
}

export function nighthawkBaseUrl(): string {
  ...
  return (process.env['NIGHTHAWK_BASE_URL'] ?? DEFAULT_NIGHTHAWK_BASE_URL).replace(/\/+$/, '');
}

export function nighthawkUsageUrl(): string {
  return `${nighthawkBaseUrl()}/usages`;
}

/**
 * Strict match against the managed NightHawk endpoint: both URLs are parsed
 ...
 */
export function isManagedNighthawkBaseUrl(baseUrl: string | undefined): boolean {
  if (baseUrl === undefined) return false;
  const managed = parseNormalizedUrl(nighthawkBaseUrl());
    ...
    if (!res.ok) {
      ...
      const hint =
        status === 401
          ...
          : status === 404
            ? 'Usage endpoint not available. Try NightHawk For Coding.'
