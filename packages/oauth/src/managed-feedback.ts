/**
 * Submit user feedback to the managed NightHawk platform.
 *
 * POSTs a JSON body to `{nighthawkBaseUrl}/feedback` with a Bearer access
 * token. The client tags `version` with a `nighthawk-` prefix so the
...
export function nighthawkFeedbackUrl(baseUrl?: string): string {
  return `${(baseUrl ?? nighthawkBaseUrl()).replace(/\/+$/, '')}/feedback`;
}
