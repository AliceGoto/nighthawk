 ...
 * `{nighthawkBaseUrl}/tools` with a Bearer access token, the same wire
...
export function nighthawkToolsUrl(baseUrl?: string): string {
  return `${(baseUrl ?? nighthawkBaseUrl()).replace(/\/+$/, '')}/tools`;
}
