...
export function nighthawkFeedbackUploadUrl(baseUrl?: string): string {
  return `${feedbackBaseUrl(baseUrl)}/feedback/upload_url`;
}

export function nighthawkFeedbackUploadCompleteUrl(baseUrl?: string): string {
...
export async function fetchCreateFeedbackUploadUrl(
  ...
  const result = await postJson(nighthawkFeedbackUploadUrl(opts.baseUrl), accessToken, body, opts);
...
export async function fetchCompleteFeedbackUpload(
  ...
  const result = await postJson(nighthawkFeedbackUploadCompleteUrl(opts.baseUrl), accessToken, body, opts);
  if (result.kind === 'error') return result;
  return { kind: 'ok' };
}
...
function feedbackBaseUrl(baseUrl?: string): string {
  return (baseUrl ?? nighthawkBaseUrl()).replace(/\/+$/, '');
}
