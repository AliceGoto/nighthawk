 ...
 * `~/.nighthawk/migration-report.json` and surfaced in the terminal summary.
 */
...
export interface MigrationSummary {
  readonly config: {
    ...
    /** Top-level keys dropped because nighthawk's config schema lacks them. */
    ...
    /** A `config.toml` conflict forced a `config.migrated-from-legacy-cli.toml` sibling. */
    readonly wroteSiblingDueToConflict: boolean;
    /** A `tui.toml` conflict forced a `tui.migrated-from-legacy-cli.toml` sibling. */
    ...
    /** Count of kimi-cli hook entries dropped because nighthawk's schema rejects them. */
    readonly droppedHooks: number;
    /**
     * When `wroteSiblingDueToConflict` is true, what landed in
     * `config.migrated-from-legacy-cli.toml` instead of the live `config.toml`.
  ...
  readonly mcp: {
    ...
    /** Source servers dropped because nighthawk's MCP schema rejects them. */
...
export interface MigrationNotices {
   ...
   * nighthawk instead. Empty when the legacy install had no OAuth login.
   */
