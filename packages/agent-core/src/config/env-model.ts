...
export const ENV_MODEL_PROVIDER_KEY = '__nighthawk_env__';
export const ENV_MODEL_ALIAS_KEY = '__nighthawk_env_model__';
...
/** Default context window (256K) used when NIGHTHAWK_MODEL_MAX_CONTEXT_SIZE is unset. */
const DEFAULT_MAX_CONTEXT_SIZE = 262144;

/** Default capabilities when NIGHTHAWK_MODEL_CAPABILITIES is unset (nighthawk models support both). */
...
function fail(message: string): never {
  throw new NighthawkError(ErrorCodes.CONFIG_INVALID, message);
}
...
function parseProviderType(raw: string | undefined): ProviderType {
  if (raw === undefined) return 'nighthawk';
  const normalized = raw.toLowerCase() as ProviderType;
  if (!ALLOWED_TYPES.includes(normalized)) {
    fail(
      `NIGHTHAWK_MODEL_PROVIDER_TYPE must be one of ${ALLOWED_TYPES.join(', ')}, got "${raw}".`,
    );
  }
  return normalized;
}
...
// fails fast like the other NIGHTHAWK_MODEL_* values, instead of silently keeping
...
/**
 * When `NIGHTHAWK_MODEL_NAME` is set, synthesize one provider + one model alias from
 * the `NIGHTHAWK_MODEL_*` environment variables and make it the default model.
...
export function applyEnvModelConfig(config: NighthawkConfig, env: Env = process.env): NighthawkConfig {
  ...
  const maxOutputRaw = trimmed(env['NIGHTHAWK_MODEL_MAX_OUTPUT_SIZE']);
  const maxOutputSize =
    maxOutputRaw !== undefined
      ? parsePositiveInt(maxOutputRaw, 'NIGHTHAWK_MODEL_MAX_OUTPUT_SIZE')
      : undefined;
  const capabilities = parseCapabilities(env['NIGHTHAWK_MODEL_CAPABILITIES']) ?? DEFAULT_CAPABILITIES;
  const displayName = trimmed(env['NIGHTHAWK_MODEL_DISPLAY_NAME']);
  const reasoningKey = trimmed(env['NIGHTHAWK_MODEL_REASONING_KEY']);
  const adaptiveThinking = parseBooleanVar(
    env['NIGHTHAWK_MODEL_ADAPTIVE_THINKING'],
    'NIGHTHAWK_MODEL_ADAPTIVE_THINKING',
  );
  ...
  // `validateConfig` throws NighthawkError(CONFIG_INVALID) on violation, matching
 ...
 */
export function stripEnvModelConfig(config: NighthawkConfig): NighthawkConfig {
...
function rawDefaultModel(config: NighthawkConfig): string | undefined {
  const raw = config.raw?.['default_model'];
  return typeof raw === 'string' ? raw : undefined;
}

function rawThinking(config: NighthawkConfig): ThinkingConfig | undefined {
