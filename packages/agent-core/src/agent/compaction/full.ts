...
export class FullCompaction {
    ...
    this.strategy =
      ...
      new DefaultCompactionStrategy(
        () => this.getEffectiveMaxContextTokens(),
        {
          ...DEFAULT_COMPACTION_CONFIG,
          reservedContextSize:
            agent.nighthawkConfig?.loopControl?.reservedContextSize ??
  ...
  begin(data: Readonly<CompactionBeginData>): void {
    ...
    if (this.agent.context.history.length === 0) {
      throw new NighthawkError(ErrorCodes.COMPACTION_UNABLE, 'No messages to compact in current history.');
    }
  ...
  private beginAutoCompaction(throwOnLimit: boolean = true): boolean {
    ...
    if (this.compactionCountInTurn >= maxCompactions) {
      if (throwOnLimit) {
        throw new NighthawkError(ErrorCodes.CONTEXT_OVERFLOW, `Compaction limit exceeded (${String(maxCompactions)})`, {
    ...
    try {
      ...
      const provider = applyCompletionBudget({
        ...
        budget: resolveCompletionBudget({
          ...
          reservedContextSize: this.agent.nighthawkConfig?.loopControl?.reservedContextSize,
        }),
        capability,
      });
      ...
      if (
        isNighthawkError(error) &&
        (error.code === ErrorCodes.AUTH_LOGIN_REQUIRED ||
          error.code === ErrorCodes.PROVIDER_AUTH_ERROR)
      )
        throw error;
      throw new NighthawkError(ErrorCodes.COMPACTION_FAILED, String(error), { cause: error });
    }
  }
