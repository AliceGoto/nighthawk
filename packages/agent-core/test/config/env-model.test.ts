...
const MIN = {
  ...
  NIGHTHAWK_MODEL_API_KEY: 'sk-test',
  NIGHTHAWK_MODEL_MAX_CONTEXT_SIZE: '262144',
} as const;

describe('applyEnvModelConfig', () => {
  it('returns the config unchanged when NIGHTHAWK_MODEL_NAME is absent', () => {
    const base = getDefaultConfig();
    expect(applyEnvModelConfig(base, {})).toBe(base);
  });

  it('throws when NIGHTHAWK_MODEL_NAME is set but API key is missing', () => {
    expectConfigInvalid(() => apply({ NIGHTHAWK_MODEL_NAME: 'm' }));
  });

  it('defaults max_context_size to 262144 (256K) when unset', () => {
    expect(
      apply({ NIGHTHAWK_MODEL_NAME: 'm', NIGHTHAWK_MODEL_API_KEY: 'k' })
  ...
  it.each(['abc', '0', '1.5', '-1'])(
    ...
    (value) => {
      expectConfigInvalid(() =>
        apply({ ...MIN, NIGHTHAWK_MODEL_MAX_CONTEXT_SIZE: value }),
      );
    },
  );

  it('synthesizes a nighthawk provider and model from the minimal set', () => {
  ...
  it('applies provider type and its default base url', () => {
    expect(apply({ ...MIN, NIGHTHAWK_MODEL_PROVIDER_TYPE: 'openai' })
      .providers[ENV_MODEL_PROVIDER_KEY]).toMatchObject({
      type: 'openai',
      baseUrl: 'https://api.openai.com/v1',
    });
    const anthropic = apply({ ...MIN, NIGHTHAWK_MODEL_PROVIDER_TYPE: 'anthropic' })
  ...
  it('rejects unsupported provider types', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, NIGHTHAWK_MODEL_PROVIDER_TYPE: 'google-genai' }),
    );
  });

  it('lets an explicit base url override the default', () => {
    expect(
      apply({ ...MIN, NIGHTHAWK_MODEL_BASE_URL: 'https://api.example.com/v1' })
  ...
  it('parses comma-separated capabilities (trimmed, lowercased)', () => {
    expect(
      apply({ ...MIN, NIGHTHAWK_MODEL_CAPABILITIES: 'Image_In, thinking ,' })
  ...
  it('sets display_name only when provided', () => {
    ...
    expect(
      apply({ ...MIN, NIGHTHAWK_MODEL_DISPLAY_NAME: 'Custom Model' })
  ...
  it('writes type-specific fields and validates max_output_size', () => {
    const alias = apply({
      ...MIN,
      NIGHTHAWK_MODEL_PROVIDER_TYPE: 'anthropic',
    ...
    expectConfigInvalid(() =>
      apply({ ...MIN, NIGHTHAWK_MODEL_MAX_OUTPUT_SIZE: 'nope' }),
    );
  });
  ...
  it('maps the thinking effort variable', () => {
    const config = apply({
      ...MIN,
      NIGHTHAWK_MODEL_THINKING_EFFORT: 'high',
    });
    expect(config.thinking).toMatchObject({ effort: 'high' });
    expect(config.thinking?.enabled).toBeUndefined();
  });

  it('maps NIGHTHAWK_MODEL_ADAPTIVE_THINKING onto the alias', () => {
    expect(
      apply({ ...MIN, NIGHTHAWK_MODEL_ADAPTIVE_THINKING: 'true' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(true);
    expect(
      apply({ ...MIN, NIGHTHAWK_MODEL_ADAPTIVE_THINKING: 'false' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(false);
    expect(
  ...
  it('rejects an invalid NIGHTHAWK_MODEL_ADAPTIVE_THINKING', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, NIGHTHAWK_MODEL_ADAPTIVE_THINKING: 'maybe' }),
    );
  });
...
describe('loadRuntimeConfig vs readConfigFile (write-back isolation)', () => {
  it('injects the env model into runtime config but readConfigFile stays clean', () => {
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-env-model-'));
...
describe('stripEnvModelConfig (write-back guard)', () => {
  ...
  it('keeps user providers/models and a non-env default_model', () => {
    ...
    config.providers['nighthawk'] = { type: 'nighthawk', apiKey: 'k', baseUrl: 'https://x/v1' };
    config.providers[ENV_MODEL_PROVIDER_KEY] = { type: 'nighthawk', apiKey: 'env-key' };
    config.models = {
      'my-model': { provider: 'nighthawk', model: 'm', maxContextSize: 1000 },
    ...
    expect(stripped.providers['nighthawk']).toBeDefined();
...
describe('writeConfigFile never persists the env model', () => {
  it('strips env entries (incl. thinking) when a runtime config is written back', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-env-write-'));
    const path = join(dir, 'config.toml');
    writeFileSync(
      path,
      'default_model = "x"\n[thinking]\neffort = "medium"\n[providers.x]\ntype = "nighthawk"\napi_key = "k"\n[models.x]\nprovider = "x"\nmodel = "x"\nmax_context_size = 1000\n',
  ...
  it('output never contains env-injected identifiers', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-env-write2-'));
    const path = join(dir, 'config.toml');
    writeFileSync(
      path,
      'default_model = "x"\n[providers.x]\ntype = "nighthawk"\napi_key = "k"\n[models.x]\nprovider = "x"\nmodel = "x"\nmax_context_size = 1000\n',
    ...
    try {
      const runtime = loadRuntimeConfig(path, {
        ...MIN,
        NIGHTHAWK_MODEL_THINKING_EFFORT: 'low',
      });
      await writeConfigFile(path, runtime);
