...
describe('findExistingRg', () => {
  ...
  beforeEach(() => {
    fakeShare = join(tmpdir(), `nighthawk-rg-${String(Date.now())}-${String(Math.random()).slice(2)}`);
...
describe('verifyArchiveChecksum', () => {
  let fakeDir: string;
  beforeEach(() => {
    fakeDir = join(tmpdir(), `nighthawk-rg-sha-${String(Date.now())}-${String(Math.random()).slice(2)}`);
...
describe('ensureRgPath download branch', () => {
  ...
  beforeEach(() => {
    fakeShare = join(
      tmpdir(),
      `nighthawk-rg-dl-${String(Date.now())}-${String(Math.random()).slice(2)}`,
    );
    mkdirSync(join(fakeShare, 'bin'), { recursive: true });
    savedPath = process.env['PATH'];
  ...
  it('downloads from the official BurntSushi releases when the env pins the overseas region', async () => {
  ...
  it('downloads from the official BurntSushi releases by default (no env override, no install marker)', async () => {
    ...
    process.env['NIGHTHAWK_HOME'] = fakeShare;
    try {
      const body = bodyFromBuffer(Buffer.from('not a real archive', 'utf8'));
      const fetchMock = vi.fn().mockResolvedValue({
        ok: true,
      ...
      if (savedLegacyHost === undefined) delete process.env['NIGHTHAWK_OAUTH_HOST'];
      else process.env['NIGHTHAWK_OAUTH_HOST'] = savedLegacyHost;
      if (savedHome === undefined) delete process.env['NIGHTHAWK_HOME'];
      else process.env['NIGHTHAWK_HOME'] = savedHome;
    }
  });
...
describe('ensureRgPath Windows download branch', () => {
  ...
  beforeEach(() => {
    fakeShare = join(
      tmpdir(),
      `nighthawk-rg-win-${String(Date.now())}-${String(Math.random()).slice(2)}`,
    );
    mkdirSync(join(fakeShare, 'bin'), { recursive: true });
    savedPath = process.env['PATH'];
