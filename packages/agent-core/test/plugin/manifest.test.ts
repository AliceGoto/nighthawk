...
async function makePlugin(
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'nighthawk-plugin-test-'));
...
describe('parseManifest', () => {
  it('reads a minimal nighthawk.plugin.json at the plugin root', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', version: '1.0.0' }),
    });
    const result = await parseManifest(root);
    expect(result.manifest?.name).toBe('demo');
    expect(result.manifest?.version).toBe('1.0.0');
    expect(result.manifestKind).toBe('nighthawk-plugin-root');
    expect(result.diagnostics).toEqual([]);
  });

  it('prefers root nighthawk.plugin.json when .nighthawk-plugin/plugin.json also exists', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'root-version', version: '1.0.0' }),
      '.nighthawk-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
    expect(result.manifestKind).toBe('nighthawk-plugin-root');
    expect(result.manifest?.name).toBe('root-version');
    expect(result.shadowedManifestPath).toBe(path.join(root, '.nighthawk-plugin/plugin.json'));
  });

  it('falls back to .nighthawk-plugin/plugin.json when nighthawk.plugin.json is absent', async () => {
    const root = await makePlugin(
      {
        '.nighthawk-plugin/plugin.json': JSON.stringify({
          name: 'demo',
          version: '1.0.0',
          keywords: ['workflow'],
          skills: './skills/',
          ...
          skillInstructions: 'Use NightHawk tools.',
        }),
      },
      { dirs: ['skills'] },
    );
    const result = await parseManifest(root);
    expect(result.manifestKind).toBe('nighthawk-plugin-dir');
    expect(result.manifestPath).toBe(path.join(root, '.nighthawk-plugin/plugin.json'));
    expect(result.manifest?.name).toBe('demo');
    expect(result.manifest?.version).toBe('1.0.0');
    ...
    expect(result.manifest?.skillInstructions).toBe('Use NightHawk tools.');
  });

  it('does NOT fall back to .nighthawk-plugin/plugin.json when nighthawk.plugin.json is invalid JSON', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': '{ not json',
      '.nighthawk-plugin/plugin.json': JSON.stringify({ name: 'dir-version' }),
    });
    const result = await parseManifest(root);
  ...
  it('rejects names that violate the regex', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'Bad Name!' }),
    });
    const result = await parseManifest(root);
    expect(result.manifest).toBeUndefined();
    expect(result.diagnostics).toContainEqual(
  ...
  it('resolves a single skills path', async () => {
    const root = await makePlugin(
      { 'nighthawk.plugin.json': JSON.stringify({ name: 'demo', skills: './skills/' }) },
      { dirs: ['skills'] },
    );
    const result = await parseManifest(root);
    expect(result.manifest?.skills).toEqual([path.join(root, 'skills')]);
  ...
  it('resolves an array of skills paths', async () => {
    const root = await makePlugin(
      {
        'nighthawk.plugin.json': JSON.stringify({
          name: 'demo',
          skills: ['./a/', './b/'],
        }),
      },
  ...
  it('rejects a skills path that escapes plugin_root', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', skills: './../escape' }),
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
      expect.objectContaining({
  ...
  it('rejects a skills path that escapes via a symlink', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', skills: './sym' }),
    });
    const outside = await mkdtemp(path.join(tmpdir(), 'nighthawk-plugin-outside-'));
  ...
  it('warns when skills resolves to a non-directory', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', skills: './notes.md' }),
      'notes.md': 'hi',
    });
    const result = await parseManifest(root);
    expect(result.diagnostics).toContainEqual(
  ...
  it('falls back to root SKILL.md when skills field is absent', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo' }),
      'SKILL.md': '---\nname: root-skill\n---\nbody',
    });
    const result = await parseManifest(root);
  ...
  it('resolves an explicit agents path', async () => {
    const root = await makePlugin(
      { 'nighthawk.plugin.json': JSON.stringify({ name: 'demo', agents: './agents/' }) },
      { dirs: ['agents'] },
    );
    const result = await parseManifest(root);
  ...
  it('parses skillInstructions', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', skillInstructions: 'Do this.' }),
    });
    const result = await parseManifest(root);
  ...
  it('parses keywords metadata', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', keywords: ['finance', 'workflow'] }),
    });
    const result = await parseManifest(root);
  ...
  it('warns when hooks is not an array', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', hooks: { event: 'Stop', command: 'x' } }),
    });
    const result = await parseManifest(root);
    expect(result.manifest?.hooks).toBeUndefined();
    expect(result.diagnostics).toContainEqual(
  ...
  it('resolves a commands directory to its .md files', async () => {
    const root = await makePlugin(
      {
        'nighthawk.plugin.json': JSON.stringify({ name: 'demo', commands: ['./commands'] }),
        'commands/deploy.md': '---\ndescription: Deploy\n---\nbody',
        'commands/env.md': '---\ndescription: Env\n---\nbody',
  ...
  it('accepts a single command .md file', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', commands: ['./deploy.md'] }),
      'deploy.md': '---\ndescription: Deploy\n---\nbody',
    });
    const result = await parseManifest(root);
    expect(result.manifest?.commands).toEqual([
  ...
  it('warns when commands is not a string or string[]', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', commands: { nope: true } }),
    });
    const result = await parseManifest(root);
  ...
  it('warns when a commands entry resolves outside the plugin', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', commands: ['../outside.md'] }),
    });
    const result = await parseManifest(root);
    expect(result.manifest?.commands).toBeUndefined();
    expect(result.diagnostics).toContainEqual(expect.objectContaining({ severity: 'warn' }));
  ...
  it('reads the systemPrompt field, trimming surrounding whitespace', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', systemPrompt: '\nAlways cite sources.\n' }),
    });
    const result = await parseManifest(root);
  ...
  it('treats a missing, blank, or non-string systemPrompt as absent', async () => {
    ...
    const nonString = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', systemPrompt: 42 }),
    });
  ...
  it('strips a UTF-8 BOM from the systemPromptPath file before trimming', async () => {
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', systemPromptPath: './PROMPT.md' }),
  ...
  it('warns on invalid systemPromptPath and keeps the inline systemPrompt', async () => {
    const nonString = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', systemPrompt: 'Inline.', systemPromptPath: 42 }),
    });
    const nonStringResult = await parseManifest(nonString);
  ...
  it('rejects systemPromptPath values that escape the plugin root', async () => {
    const traversal = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', systemPromptPath: './../outside.md' }),
    });
    ...
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', systemPromptPath: './linked.md' }),
    });
  ...
  it('ignores an oversized inline systemPrompt with a warning', async () => {
    ...
    const root = await makePlugin({
      'nighthawk.plugin.json': JSON.stringify({ name: 'demo', systemPrompt: oversized }),
    });
    const result = await parseManifest(root);
