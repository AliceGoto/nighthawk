...
describe('extractZip', () => {
  ...
  it('prefers a plugin manifest at the archive root', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'nighthawk.plugin.json', data: '{"name":"root"}' },
      { name: 'examples/demo/nighthawk.plugin.json', data: '{"name":"demo"}' },
    ]);
  ...
  it('detects plugin root with nighthawk.plugin.json', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'my-plugin/nighthawk.plugin.json', data: '{"name":"test"}' },
      { name: 'my-plugin/readme.md', data: '# Test' },
    ]);

    const root = await extractZip(zipBuffer, destDir);
    expect(root).toBe(path.join(destDir, 'my-plugin'));
    const manifest = await readFile(path.join(root, 'nighthawk.plugin.json'), 'utf8');
    expect(manifest).toBe('{"name":"test"}');
  });

  it('detects plugin root with .nighthawk-plugin/plugin.json', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'my-plugin/.nighthawk-plugin/plugin.json', data: '{"name":"test"}' },
      { name: 'my-plugin/skills/demo/SKILL.md', data: '---\nname: demo\n---\nbody' },
    ]);

    const root = await extractZip(zipBuffer, destDir);
    expect(root).toBe(path.join(destDir, 'my-plugin'));
    const manifest = await readFile(path.join(root, '.nighthawk-plugin', 'plugin.json'), 'utf8');
    expect(manifest).toBe('{"name":"test"}');
  });

  it('does not treat legacy kimi manifests as plugin roots', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const rootLevel = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const rootZip = await createZipBuffer([
      { name: 'kimi.plugin.json', data: '{"name":"kimi-root"}' },
      { name: 'readme.md', data: '# Kimi' },
    ]);
    const dirZip = await createZipBuffer([
      { name: 'my-plugin/.kimi-plugin/plugin.json', data: '{"name":"kimi-dir"}' },
      { name: 'my-plugin/readme.md', data: '# Kimi' },
    ]);

    expect(await extractZip(rootZip, rootLevel)).toBe(rootLevel);
    expect(await extractZip(dirZip, destDir)).toBe(destDir);
  });

  it('detects a single wrapper directory before nested manifests', async () => {
    const destDir = await mkdtemp(path.join(tmpdir(), 'archive-test-'));
    const zipBuffer = await createZipBuffer([
      { name: 'outer/nighthawk.plugin.json', data: '{"name":"outer"}' },
      { name: 'outer/inner/nighthawk.plugin.json', data: '{"name":"inner"}' },
    ]);
