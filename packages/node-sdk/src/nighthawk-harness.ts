...
import type { NighthawkAuthFacade } from '#/auth';
...
export interface NighthawkHarnessRuntimeOptions {
...
export class NighthawkHarness {
  readonly homeDir: string;
  readonly configPath: string;
  readonly auth: NighthawkAuthFacade;

  private readonly identity: NighthawkHostIdentity | undefined;
  ...
  constructor(
    ...
    options: NighthawkHarnessRuntimeOptions,
  ) {
   ...
   * capabilities nighthawk-cu / nighthawk-webbridge), no session required. Routed
  ...
  async getConfig(options: GetConfigOptions = {}): Promise<NighthawkConfig> {
  ...
  async setConfig(patch: NighthawkConfigPatch): Promise<NighthawkConfig> {
    return this.rpc.setConfig(patch);
  }

  async removeProvider(providerId: string): Promise<NighthawkConfig> {
  ...
  /**
   * The unified MCP management view: user-level `<NIGHTHAWK_HOME>/mcp.json`
  ...
  async authenticateMcpServer(
    ...
    try {
      ...
      if (opened === false) {
        throw new NighthawkError(ErrorCodes.REQUEST_INVALID, 'MCP OAuth authorization was cancelled');
      }
...
function normalizeSessionId(value: string): string {
  if (typeof value !== 'string') {
    throw new NighthawkError(ErrorCodes.SESSION_ID_REQUIRED, 'Session id is required.');
  }
  const normalized = value.trim();
  if (normalized.length === 0) {
    throw new NighthawkError(ErrorCodes.SESSION_ID_EMPTY, 'Session id cannot be empty.');
  }
  return normalized;
}
