 ...
 * (DI × Scope) instead of the v1 `NighthawkCore` RPC pair. The engine is
 ...
 *   `klient.global.config.*`, with the v1 `NighthawkConfig` shape restored by the
...
import { encodeWorkDirKey } from '@nighthawk/agent-core-v2/_base/utils/workdir-slug';
import { McpConnectionManager } from '@nighthawk/agent-core-v2/mcpCore/connection-manager';
import { loadMcpServers } from '@nighthawk/agent-core-v2/app/mcpConfig/configLoader';
import { IAppendLogStore } from '@nighthawk/agent-core-v2/persistence/interface/appendLogStore';
import type { McpServerConfig as WorkspaceMcpServerConfig } from '@nighthawk/agent-core-v2/mcpCore/config-schema';
import {
  bootstrap,
...
import type { AgentHandle, Klient } from '@nighthawk/klient';
import { createKlient } from '@nighthawk/klient/memory';
...
import {
  ...
  planProviderRemoval,
  resolvedConfigToNighthawkConfig,
...
export class SDKRpcClientV2 extends SDKRpcClientBase {
  ...
  constructor(options: SDKRpcClientV2Options = {}) {
    ...
    ensureNighthawkHome(this.homeDir);
    ...
    const identity = assertNighthawkHostIdentity(this.identity);
    const { app } = bootstrap(
      {
        homeDir: this.homeDir,
        configPath: this.configPath,
        ...
        args: {
          ...
          requestHeaders: createNighthawkDefaultHeaders({ homeDir: this.homeDir, ...identity }),
   ...
   * client only served `NighthawkHarness`-level events and every engine-side event
  ...
  /**
   * v1 returns the whole config.toml document as one `NighthawkConfig`; v2
   ...
   * runtime config (`loadRuntimeConfigSafe` + the NIGHTHAWK_MODEL_* overlay);
   * `reload` mirrors v1's re-read-from-disk option.
   */
  override async getConfig(options?: GetConfigOptions): Promise<NighthawkConfig> {
    await this.configReady;
    if (options?.reload) {
      await this.klient.global.config.reload();
    }
    return resolvedConfigToNighthawkConfig(await this.klient.global.config.getAll());
  }
   ...
   */
  override async setConfig(patch: NighthawkConfigPatch): Promise<NighthawkConfig> {
    await this.configReady;
   ...
   * `removeNighthawkProvider`, so a process exit can never leave the file in a
   * halfway-cascaded state.
   */
  override async removeProvider(providerId: string): Promise<NighthawkConfig> {
    await this.configReady;
  ...
  /**
   * Capability surface (v2-only): built-in product capabilities (nighthawk-cu,
   * nighthawk-webbridge) with layered readiness and idempotent installs. v1 has
  ...
  private static sessionNotFound(sessionId: string): NighthawkError {
    return new NighthawkError(ErrorCodes.SESSION_NOT_FOUND, `Session "${sessionId}" was not found`, {
      details: { sessionId },
    });
  }
  ...
  override async renameSession(input: RenameSessionInput): Promise<void> {
    ...
    if (title.length === 0) {
      throw new NighthawkError(ErrorCodes.SESSION_TITLE_EMPTY, 'Session title cannot be empty');
    }
   ...
   * `NighthawkError(SESSION_NOT_FOUND)` — the pre-check here keeps the v1 shape.
   ...
   * project-local `.nighthawk/local.toml`, `persist: false` joins the
  ...
  private async materializeMainAgent(
    ...
    if (agent === undefined) {
      throw new NighthawkError(ErrorCodes.AGENT_NOT_FOUND, 'Main agent was not found');
    }
  ...
  private async agentScope(sessionId: string): Promise<IAgentScopeHandle> {
    ...
    if (agent === undefined) {
      throw new NighthawkError(ErrorCodes.AGENT_NOT_FOUND, `Agent "${agentId}" was not found`);
    }
    return agent;
  }
   ...
   * contract is `NighthawkError` (what `isNighthawkError` branches on, and what the v1
...
function normalizeRequiredWorkDir(operation: string, workDir: string): string {
  if (typeof workDir !== 'string' || workDir.trim() === '') {
    throw new NighthawkError(ErrorCodes.REQUEST_WORK_DIR_REQUIRED, `${operation} requires workDir`);
  }
  return normalizeWorkDir(workDir);
}

/**
 * Restate an engine `Error2` in the SDK's public error shape (`NighthawkError`,
 * what `isNighthawkError` branches on) so the delegated management plane throws
 ...
 * mint a `NighthawkError` that `toNighthawkErrorPayload` cannot serialize (its
 * `NIGHTHAWK_ERROR_INFO` lookup throws on undeclared codes).
 */
function restateMcpManagementError(error: unknown): unknown {
  if (!isError2(error)) return error;
  const code: NighthawkErrorCode = isNighthawkErrorCode(error.code) ? error.code : ErrorCodes.INTERNAL;
  return new NighthawkError(code, error.message, {
