...
export abstract class SDKRpcClientBase {
  ...
  async getConfig(input?: GetConfigOptions): Promise<NighthawkConfig> {
    const rpc = await this.getRpc();
    return rpc.getNighthawkConfig(input ?? {});
  }
  ...
  async setConfig(input: NighthawkConfigPatch): Promise<NighthawkConfig> {
    const rpc = await this.getRpc();
    return rpc.setNighthawkConfig(input);
  }

  async removeProvider(providerId: string): Promise<NighthawkConfig> {
    const rpc = await this.getRpc();
    return rpc.removeNighthawkProvider({ providerId });
  }
  ...
  async getRuntime(input: SessionIdRpcInput): Promise<AgentRuntimeBinding> {
    void input;
    throw new NighthawkError(ErrorCodes.NOT_IMPLEMENTED, 'This SDK client does not support runtimes.');
  }
