 ...
 * agent-core-v2, the v1 harness (`createNighthawkHarness`) and the v2 harness
 * (`createNighthawkHarnessV2`) must return identical values on the same fixture
 ...
 * NIGHTHAWK_HOME; no provider calls.
...
/**
 * Projection for every method returning a `NighthawkConfig`: drop the fields whose
 * presence semantics genuinely differ between the engines (see KNOWN_DIFFS).
 */
function projectConfig(config: NighthawkConfig): unknown {
...
const KNOWN_DIFFS = {
  ...
  getConfig: (config: NighthawkConfig): unknown => projectConfig(config),
  setConfig: (config: NighthawkConfig): unknown => projectConfig(config),
  removeProvider: (config: NighthawkConfig): unknown => projectConfig(config),
 ...
 *   rosters. The four security tools (`DepAudit`/`SecretScan`/
 *   `SecurityScan`/`TaintTrace`) are registered and exposed by both default
 *   profiles, so model-bound fixtures compare them on both sides. A model-less
...
interface ParityFixture {
  readonly v1: NighthawkHarness;
  readonly v2: NighthawkHarness;
  readonly homeDir: string;
}

async function makeParityPair(): Promise<ParityFixture> {
  const homeDir = await makeTempDir('nighthawk-sdk-parity-home-');
  const v1 = createNighthawkHarness({ homeDir, identity: TEST_IDENTITY });
  const v2 = createNighthawkHarnessV2({ homeDir, identity: TEST_IDENTITY });
  return { v1, v2, homeDir };
}

async function closeAll(...harnesses: readonly NighthawkHarness[]): Promise<void> {
...
/**
 * Config-reading env vars both engines honor (`NIGHTHAWK_MODEL_*`, the per-section
...
const CONFIG_ENV_PATTERN =
  /^(NIGHTHAWK_MODEL_|NIGHTHAWK_LOOP_|NIGHTHAWK_MCP_|NIGHTHAWK_WEB_|NIGHTHAWK_IMAGE_|NIGHTHAWK_BACKGROUND_|NIGHTHAWK_MODEL_CATALOG_)/;
...
async function makeIsolatedParityPair(
  configToml?: string,
): Promise<{ v1: NighthawkHarness; v2: NighthawkHarness }> {
  const v1Home = await makeTempDir('nighthawk-sdk-parity-v1-home-');
  const v2Home = await makeTempDir('nighthawk-sdk-parity-v2-home-');
  if (configToml !== undefined) {
    await writeFile(join(v1Home, 'config.toml'), configToml, 'utf-8');
    await writeFile(join(v2Home, 'config.toml'), configToml, 'utf-8');
  }
  return {
    v1: createNighthawkHarness({ homeDir: v1Home, identity: TEST_IDENTITY }),
    v2: createNighthawkHarnessV2({ homeDir: v2Home, identity: TEST_IDENTITY }),
  };
}
...
function expectConfigParity(v1Config: NighthawkConfig, v2Config: NighthawkConfig): void {
...
describe('v1↔v2 return-value parity', () => {
  ...
  it('listWorkspaceSkills returns the same skills on the same fixture', async () => {
    ...
    const workDir = await makeTempDir('nighthawk-sdk-parity-work-');
    await writeSkill(join(homeDir, 'skills', 'parity-user-skill'), 'parity-user-skill');
    await writeSkill(
      join(workDir, '.nighthawk', 'skills', 'parity-project-skill'),
...
// Plugin parity
//
// The plugin methods live on the rpc client itself (NighthawkHarness has no
...
async function writeFixturePlugin(dir: string): Promise<void> {
  ...
  await writeFile(
    join(dir, 'nighthawk.plugin.json'),
    JSON.stringify(
      {
...
async function makePluginParityPair(configToml?: string): Promise<PluginParityPair> {
  const v1HomeDir = await makeTempDir('nighthawk-sdk-parity-v1-home-');
  const v2HomeDir = await makeTempDir('nighthawk-sdk-parity-v2-home-');
  const sourceDir = await makeTempDir('nighthawk-sdk-parity-plugin-src-');
  if (configToml !== undefined) {
...
describe('v1↔v2 session lifecycle parity', () => {
  it('createSession returns the same summary for the same input', async () => {
    ...
    const extraDir = await makeTempDir('nighthawk-sdk-parity-extra-');
    try {
      const input = {
  ...
  it('listSessions reflects the same creates and renames across filters', async () => {
    const pair = await makeSessionParityPair();
    const otherWorkDir = await makeTempDir('nighthawk-sdk-parity-work-other-');
    try {
  ...
  it('addAdditionalDir returns the same result for persist and non-persist', async () => {
    const pair = await makeSessionParityPair();
    const persistedDir = await makeTempDir('nighthawk-sdk-parity-extra-persisted-');
    const sessionOnlyDir = await makeTempDir('nighthawk-sdk-parity-extra-session-');
    try {
      ...
      expect(v1Persisted).toMatchObject({
        ...
        configPath: join(pair.workDir, '.nighthawk', 'local.toml'),
        persisted: true,
      });
...
/**
 * Agent fixture: a nighthawk-typed provider (strict thinking validation on both
...
describe('v1↔v2 agent interaction parity', () => {
  ...
  it('setThinking applies and rejects unlisted efforts identically', async () => {
    ...
    try {
      ...
      // An unlisted effort on a strict-thinking (nighthawk-typed) model rejects
  ...
  it('activateSkill renders, launches, and rejects identically', async () => {
    ...
    await writeSkill(join(pair.workDir, '.nighthawk', 'skills', 'parity-skill'), 'parity-skill');
    try {
 ...
 * class is pinned because SDK consumers branch on `isNighthawkError`.
 */
...
describe('v1↔v2 global MCP parity', () => {
  it('detects implicit OAuth requirements identically by default', async () => {
    ...
    for (const homeDir of [pair.v1HomeDir, pair.v2HomeDir]) {
      const externalOAuth = new McpOAuthService({ nighthawkHomeDir: homeDir });
      await externalOAuth
  ...
  it('picks up a grant saved after the first probe when verifying', async () => {
    ...
    try {
      ...
      for (const homeDir of [pair.v1HomeDir, pair.v2HomeDir]) {
        await new McpOAuthService({ nighthawkHomeDir: homeDir })
  ...
  it.each([
    ...
    const pair = await makeGlobalMcpParityPair();
    const project = await makeTempDir('nighthawk-sdk-parity-mcp-project-');
    await mkdir(join(project, '.nighthawk'), { recursive: true });
    await writeFile(
      join(project, '.nighthawk', 'mcp.json'),
  ...
  it('threads cwd through project-layer inspection and authorization on both engines', async () => {
    const pair = await makeGlobalMcpParityPair();
    const project = await makeTempDir('nighthawk-sdk-parity-mcp-auth-project-');
    await mkdir(join(project, '.nighthawk'), { recursive: true });
    await writeFile(
      join(project, '.nighthawk', 'mcp.json'),
      JSON.stringify({
...
describe('v1↔v2 session MCP parity', () => {
  ...
  it('a persisted session add over an enabled plugin entry follows each engine\'s precedence', async () => {
    const restoreEnv = scrubConfigEnv();
    const pair = await makeSessionMcpPair();
    const pluginSource = await makeTempDir('nighthawk-sdk-parity-mcp-plugin-src-');
    ...
    try {
      ...
      // (`isNighthawkError` is the SDK's public error class on both engines). v2
      ...
      expect(isNighthawkError(v1Error)).toBe(true);
...
describe('v1↔v2 residual surface parity', () => {
  it('exportSession packages the same entry list and manifest on both engines', async () => {
    const pair = await makeSessionParityPair();
    const outDir = await makeTempDir('nighthawk-sdk-parity-export-');
    try {
  ...
  it('listSkills returns the same merged catalog on the same fixture', async () => {
    ...
    try {
      ...
      await writeSkill(
        join(pair.workDir, '.nighthawk', 'skills', 'parity-project-skill'),
        'parity-project-skill',
      );
