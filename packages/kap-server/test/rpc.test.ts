...
import { FakeRuntime } from '@nighthawk/agent-core-v2/runtime/fakeRuntime';
...
describe('server-v2 /api/v1/debug RPC', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-rpc-'));
  ...
  it('lists and installs plugins through RPC', async () => {
    ...
    try {
      ...
      await writeFile(
        join(pluginRoot, 'nighthawk.plugin.json'),
...
describe('server-v2 /api/v1/debug RPC auth', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-rpc-auth-'));
...
describe('server-v2 /api/v1/debug RPC (dev-only, whitelist-free)', () => {
  ...
  beforeEach(async () => {
    home = await mkdtemp(join(tmpdir(), 'nighthawk-server-v2-debug-rpc-'));
    server = await startServer({
      hostIdentity: TEST_HOST_IDENTITY,
      host: '127.0.0.1',
      port: 0,
