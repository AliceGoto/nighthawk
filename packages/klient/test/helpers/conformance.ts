...
import { Service } from '@nighthawk/agent-core-v2/_base/di/service';
import { CommandContribution } from '@nighthawk/agent-core-v2/agent/command/commandContribution';
import { IFeatureManager } from '@nighthawk/agent-core-v2/app/feature/featureManager';
import { getLiveSessionById } from '@nighthawk/agent-core-v2/app/sessionManager/sessionLookup';
import { IAgentLifecycleService } from '@nighthawk/agent-core-v2/session/agentLifecycle/agentLifecycle';
import { IAgentPromptService, reservePrompt } from '@nighthawk/agent-core-v2/agent/prompt/prompt';
...
export function defineKlientConformance(
  ...
  describe(`klient conformance: ${transport}`, () => {
    ...
    it('session skills.list returns the workspace skills as summaries', async () => {
      ...
      try {
        await mkdir(join(workDir, '.nighthawk', 'skills', 'conf-skill'), { recursive: true });
        await writeFile(
          join(workDir, '.nighthawk', 'skills', 'conf-skill', 'SKILL.md'),
