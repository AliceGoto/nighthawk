...
import { ErrorCodes, Error2, isCodedError, isError2, toNighthawkErrorPayload, unwrapErrorCause } from "#/errors";
...
export class AgentFullCompactionService extends Service implements IAgentFullCompactionService {
  ...
  private async compactionWorker(
    ...
    try {
      ...
      void this.dispatcher.dispatch(
        new AgentErrorEvent({ ...toNighthawkErrorPayload(error), agentId: this.agent.agentId }),
      );
      throw error;
    } finally {
      try {
