...
interface TestModelProviderOptions {
  ...
  readonly nighthawkRequestHeaders?: Record<string, string>;
}
...
export interface TestAgentOptions {
  ...
  | undefined;
  readonly initialConfig?: Partial<NighthawkConfig> | undefined;
...
export function configServices(readConfig: () => NighthawkConfig): TestAgentServiceOverride {
...
export class AgentTestContext {
  ...
  nighthawkConfig: NighthawkConfig;
  ...
  constructor(overrides: readonly TestAgentServiceOverride[] = [], options: TestAgentOptions = {}) {
    ...
    this.nighthawkConfig = applyTestAgentOptionsToConfig(emptyConfig(), options);
    ...
    const appSeeds = collectScopeSeed(
      [
        (reg) => {
          for (const [id, value] of bootstrapSeed({
            homeDir: '/tmp/nighthawk-agent-app-v2-test',
            cwd: this.cwd,
          ...
          reg.defineInstance(
            IConfigService,
            configService(() => this.nighthawkConfig),
          );
          ...
          reg.defineInstance(
            ...
            {
              level: 'off',
              globalLogPath: '/tmp/nighthawk-agent-app-v2-test/logs/nighthawk.log',
  ...
  configureRuntimeModel(
    ...
    this.nighthawkConfig = configWithProvider(this.nighthawkConfig, provider, modelCapabilities);
  ...
  async expectResumeMatches(): Promise<void> {
    ...
    await this.drainWirePersistence();
    const configSnapshot = structuredClone(this.get(IConfigService).getAll() as NighthawkConfig);
...
function emptyConfig(): NighthawkConfig {
  return configWithProvider({ providers: {} }, MOCK_PROVIDER, undefined);
}

function applyTestAgentOptionsToConfig(config: NighthawkConfig, options: TestAgentOptions): NighthawkConfig {
...
function configService(readConfig: () => NighthawkConfig): IConfigService {
...
function configWithEnvOverrides(config: NighthawkConfig): NighthawkConfig {
  const maxCompletionTokens =
    parseEnvCompletionTokens(process.env['NIGHTHAWK_MODEL_MAX_COMPLETION_TOKENS']) ??
    parseEnvCompletionTokens(process.env['NIGHTHAWK_MODEL_MAX_TOKENS']);
  const temperature = parseEnvFloat(process.env['NIGHTHAWK_MODEL_TEMPERATURE']);
  const topP = parseEnvFloat(process.env['NIGHTHAWK_MODEL_TOP_P']);
  const forcedEffort = process.env['NIGHTHAWK_MODEL_THINKING_EFFORT']?.trim();
  const thinkingKeep = process.env['NIGHTHAWK_MODEL_THINKING_KEEP']?.trim();
...
function cronEnvOverrides(base: Record<string, unknown>): Record<string, unknown> | undefined {
  ...
  setBoolean('debug', 'NIGHTHAWK_CRON_DEBUG');
  setBoolean('noJitter', 'NIGHTHAWK_CRON_NO_JITTER');
  setBoolean('noStale', 'NIGHTHAWK_CRON_NO_STALE');
  setBoolean('disabled', 'NIGHTHAWK_DISABLE_CRON');
  setBoolean('manualTick', 'NIGHTHAWK_CRON_MANUAL_TICK');
  const pollIntervalMs = parseEnvCronPollIntervalMs(process.env['NIGHTHAWK_CRON_POLL_INTERVAL_MS']);
  if (pollIntervalMs !== undefined) {
    next['pollIntervalMs'] = pollIntervalMs;
    changed = true;
  }
  if (process.env['NIGHTHAWK_CRON_CLOCK'] !== undefined) {
    next['clock'] = process.env['NIGHTHAWK_CRON_CLOCK'];
    changed = true;
  }
...
function providerConfigForAlias(provider: TestProviderConfig): NighthawkConfig['providers'][string] {
  return {
    type: provider.type,
