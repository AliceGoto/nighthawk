...
const OAUTH_PROVIDER = 'managed:nighthawk';
...
describe('OAuthService', () => {
  ...
  it('startLogin resolves an env-scoped oauth ref for the managed provider without oauth config', async () => {
    providers[OAUTH_PROVIDER] = { type: 'nighthawk', baseUrl: 'https://api.example.com' };
    stubManagedModelsFetch();
    toolkit.login.mockImplementation((_provider, options) => {
      options.onDeviceCode(deviceAuth);
      return Promise.resolve({ providerName: OAUTH_PROVIDER, ok: true });
  ...
  it('startLogin honors NIGHTHAWK_BASE_URL / NIGHTHAWK_OAUTH_HOST for the login environment', async () => {
    vi.stubEnv('NIGHTHAWK_BASE_URL', 'https://env-api.example.com/coding/v1');
    vi.stubEnv('NIGHTHAWK_OAUTH_HOST', 'https://env-auth.example.com');
    stubManagedModelsFetch();
    toolkit.login.mockImplementation((_provider, options) => {
      options.onDeviceCode(deviceAuth);
      return Promise.resolve({ providerName: OAUTH_PROVIDER, ok: true });
  ...
  it('getRegion resolves cn by default and global from the persisted login host', () => {
    vi.stubEnv('NIGHTHAWK_REGION_MARKER', 'off');
    const svc = createService();
  ...
  it('getRegion reads the install marker from the bootstrapped home unless NIGHTHAWK_REGION_MARKER=off', async () => {
    const home = ix.get(IBootstrapService).homeDir;
    try {
      await mkdir(home, { recursive: true });
      await writeFile(join(home, 'region'), 'global\n', 'utf-8');
      vi.stubEnv('NIGHTHAWK_OAUTH_HOST', '');
      providers[OAUTH_PROVIDER] = { type: 'nighthawk' };
  ...
  it('getRegion reads the marker from the bootstrapped home, not NIGHTHAWK_HOME', async () => {
    const bootstrapHome = ix.get(IBootstrapService).homeDir;
    const envHome = await mkdtemp(join(tmpdir(), 'nighthawk-v2-auth-envhome-'));
    try {
      await mkdir(bootstrapHome, { recursive: true });
      await writeFile(join(bootstrapHome, 'region'), 'global\n', 'utf-8');
      vi.stubEnv('NIGHTHAWK_HOME', envHome);
      vi.stubEnv('NIGHTHAWK_OAUTH_HOST', '');
      providers[OAUTH_PROVIDER] = { type: 'nighthawk' };
      expect(createService().getRegion()).toBe('global');
    } finally {
  ...
  it('startLogin returns authenticated when login resolves without issuing a device code (already-authenticated fast path)', async () => {
    ...
    expect(configReplace).toHaveBeenCalledWith('defaultModel', 'nighthawk/nighthawk-k2');
  });
  ...
  it('refreshes managed models and sets the default model after a device-code login succeeds', async () => {
    ...
    expect(configReplace).toHaveBeenCalledWith(
      'models',
      expect.objectContaining({
        'nighthawk/nighthawk-k2': expect.objectContaining({ model: 'nighthawk-k2' }),
      }),
    );
    expect(configReplace).toHaveBeenCalledWith('defaultModel', 'nighthawk/nighthawk-k2');
  });
  ...
  it('logout removes managed provider models and dangling defaults', async () => {
    ...
    defaultModel = 'nighthawk/nighthawk-k2';
  ...
  it('resolveTokenProvider re-derives the managed provider oauth ref from the current base url', () => {
    ...
    const expectedRef = resolveNighthawkRuntimeAuth({
  ...
  it('refreshOAuthProviderModels returns an empty result when no NightHawk provider is configured', async () => {
  ...
  it('refreshOAuthProviderModels fetches models and writes back the changed sections', async () => {
    ...
    expect(configReplace).toHaveBeenCalledWith(
      'providers',
      expect.objectContaining({ [OAUTH_PROVIDER]: expect.objectContaining({ type: 'nighthawk' }) }),
    );
    expect(configReplace).toHaveBeenCalledWith(
      'models',
      expect.objectContaining({
...
describe('WebSearchProviderService', () => {
  ...
  it('returns undefined when the managed provider is not an OAuth nighthawk provider', () => {
    providers = { [OAUTH_PROVIDER]: { type: 'nighthawk', apiKey: 'sk-test' } };
    expect(createService().getWebSearchProvider()).toBeUndefined();
    expect(resolveTokenProvider).not.toHaveBeenCalled();
  });
  ...
  it('searches against /search with the OAuth access token, host identity headers, and custom headers', async () => {
    ...
    expect(headers['User-Agent']).toBe('nighthawk-cli/test');
    expect(headers['X-Msh-Device-Id']).toBe('device-test');
    expect(headers['X-Custom']).toBe('yes');
    expect(JSON.parse(init.body as string)).toEqual({ text_query: 'hello' });
  });

  it('builds a search provider from the services.nighthawk_search api_key config', async () => {
    servicesConfig = {
      nighthawkSearch: {
  ...
  it('prefers the services.nighthawk_search config over the managed oauth provider', async () => {
    servicesConfig = {
      nighthawkSearch: { baseUrl: 'https://config.example.com/search', apiKey: 'config-key' },
    };
    providers = {
      [OAUTH_PROVIDER]: {
        type: 'nighthawk',
  ...
  it('builds a search provider from the services.nighthawk_search oauth ref', async () => {
    servicesConfig = {
      nighthawkSearch: {
        baseUrl: 'https://search.example.com/search',
        oauth: { storage: 'file', key: 'oauth/nighthawk' },
  ...
  it('returns undefined when services.nighthawk_search has no baseUrl and no managed oauth', () => {
    servicesConfig = { nighthawkSearch: { apiKey: 'search-key' } };
    expect(createService().getWebSearchProvider()).toBeUndefined();
    expect(resolveTokenProvider).not.toHaveBeenCalled();
  });
  ...
  it('answers presence without touching a not-yet-frozen identity', () => {
    ...
    servicesConfig = {
      nighthawkSearch: { baseUrl: 'https://search.example.com/search', apiKey: 'k' },
    };
...
describe('services config section', () => {
  it('registers the services section and validates its schema', () => {
    ...
    expect(
      registry.validate(SERVICES_SECTION, {
        nighthawkSearch: { baseUrl: 'https://api.example.com/search', apiKey: 'search-key' },
        nighthawkFetch: { baseUrl: 'https://api.example.com/fetch' },
    ...
    expect(() =>
      registry.validate(SERVICES_SECTION, { nighthawkSearch: { baseUrl: 42 } }),
    ).toThrow();
  });
    ...
    expect(
      servicesFromToml({
        nighthawk_search: {
          ...
          oauth: { storage: 'file', key: 'oauth/nighthawk', oauth_host: 'https://auth.example.com' },
        },
        nighthawk_fetch: { base_url: 'https://api.example.com/fetch', api_key: 'fetch-key' },
      }),
    ).toEqual({
      nighthawkSearch: {
        baseUrl: 'https://api.example.com/search',
        ...
        oauth: { storage: 'file', key: 'oauth/nighthawk', oauthHost: 'https://auth.example.com' },
      },
      nighthawkFetch: { baseUrl: 'https://api.example.com/fetch', apiKey: 'fetch-key' },
    });
  });
  ...
  it('preserves unknown services when managed services are removed', () => {
    ...
    clearManagedNighthawkConfig(config);
...
describe('AuthSummaryService', () => {
  ...
  it('summarize skips providers whose status throws', async () => {
    const OTHER_OAUTH = 'nighthawk-anthropic';
  ...
  it('ensureReady throws model_not_resolved when the model provider is missing', async () => {
    ...
    await expect(createSummary().ensureReady()).rejects.toMatchObject({
      code: 'auth.model_not_resolved',
      details: { model_id: 'nighthawk', provider_id: OAUTH_PROVIDER },
    });
    expect(getCachedAccessToken).not.toHaveBeenCalled();
  });
  ...
  it('ensureReady accepts cached oauth tokens', async () => {
    ...
    await expect(createSummary().ensureReady('nighthawk')).resolves.toBeUndefined();
    expect(getCachedAccessToken).toHaveBeenCalledWith(OAUTH_PROVIDER, {
      storage: 'file',
      key: 'oauth/nighthawk',
    });
...
describe('AuthLegacyService', () => {
  ...
  it('counts every configured provider, not only oauth ones', async () => {
    providers = {
      [OAUTH_PROVIDER]: { type: 'nighthawk', oauth: { storage: 'file', key: 'oauth/nighthawk' } },
      [NON_OAUTH_PROVIDER]: { type: 'openai', apiKey: 'sk-test' },
    };
  ...
  it('reflects the configured default model', async () => {
    providers = { [NON_OAUTH_PROVIDER]: { type: 'nighthawk', apiKey: 'sk-test' } };
    defaultModel = 'k2';
    const summary = await createService().get();
