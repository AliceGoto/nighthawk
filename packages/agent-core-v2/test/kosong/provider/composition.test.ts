...
describe('supportedProtocols (probe 4)', () => {
  it('is derived from the registered bases and contains neither nighthawk nor vertexai', () => {
    ...
    expect(protocols).not.toContain('nighthawk');
    expect(protocols).not.toContain('vertexai');
  });
});

describe('apiKey env suppression (probe 1)', () => {
  it('does not pick up OPENAI_API_KEY when composing nighthawk without any key', async () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'nighthawk',
      modelName: 'nighthawk-k2',
    });
  ...
  it('uses the NIGHTHAWK_API_KEY env fallback when composing nighthawk', () => {
    process.env['NIGHTHAWK_API_KEY'] = 'sk-nighthawk-from-env';
    ...
    expect(apiKey).toBe('sk-nighthawk-from-env');
  });
...
describe('resolveAdapterIdentity', () => {
  it('resolves the (nighthawk, openai) pair registration: its traits plus the trailing synthetic trait', () => {
    const identity = registry.resolveAdapterIdentity('openai', 'nighthawk');
    expect(identity.baseId).toBe('openai');
    expect(identity.traits).toHaveLength(2);
  });

  it('resolves the (nighthawk, anthropic) pair registration: only its own traits', () => {
    const identity = registry.resolveAdapterIdentity('anthropic', 'nighthawk');
    expect(identity.baseId).toBe('anthropic');
    expect(identity.traits).toHaveLength(2);
  });

  it('resolves an unregistered (vendor, protocol) pair to no vendor traits', () => {
    const identity = registry.resolveAdapterIdentity('google-genai', 'nighthawk');
...
describe('resolveProviderBaseId', () => {
  it('returns the pair registration’s baseProtocol — the protocol itself by construction', () => {
    expect(registry.resolveProviderBaseId('openai', 'nighthawk')).toBe('openai');
    expect(registry.resolveProviderBaseId('anthropic', 'nighthawk')).toBe('anthropic');
  });

  it('returns the protocol itself otherwise', () => {
    expect(registry.resolveProviderBaseId('google-genai', 'nighthawk')).toBe('google-genai');
...
describe('resolveCapability', () => {
  ...
  it('nighthawk declares no vendor-level capability — the base catalog answers instead', () => {
    expect(isUnknownCapability(registry.resolveCapability('openai', 'nighthawk', 'nighthawk'))).toBe(
      true,
    );
...
describe('createChatProvider', () => {
  it('composes nighthawk as the openai base with the upload capability bound', () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'nighthawk',
      modelName: 'nighthawk-k2',
    ...
    expect(provider.modelName).toBe('nighthawk-k2');
...
describe('resolveProviderEndpoint', () => {
  it('resolves the nighthawk endpoint chain from process.env', () => {
  ...
  it('reads a caller-supplied env bag instead of process.env', () => {
    process.env['NIGHTHAWK_API_KEY'] = 'sk-nighthawk-env';
    expect(resolveProviderEndpoint('nighthawk', { NIGHTHAWK_BASE_URL: 'https://example.com/v1' })).toEqual({
...
describe('nighthawk provider definitions', () => {
  it('registers one definition per transport, with shared vendor-level facts', () => {
    const native = getProviderDefinition('nighthawk', 'openai');
    const anthropic = getProviderDefinition('nighthawk', 'anthropic');
  ...
  it('answers id-level queries and reports unregistered pairs', () => {
    expect(getProviderDefinition('nighthawk')?.baseProtocol).toBe('openai');
    expect(getProviderDefinitions('nighthawk')).toHaveLength(2);
    expect(hasProviderDefinition('nighthawk')).toBe(true);
    expect(hasProviderDefinition('no-such-vendor')).toBe(false);
    expect(getProviderDefinition('nighthawk', 'google-genai')).toBeUndefined();
  });
  ...
  it('allows the same id on several protocols but rejects a duplicate (id, baseProtocol) pair', () => {
    ...
    expect(() =>
      registerProviderDefinition({ id: 'nighthawk', baseProtocol: 'openai', traits: [] }),
    ).toThrow(/already registered/);
  });
});
...
describe('per-turn intent wire encoding (behavior probes)', () => {
  ...
  it('encodes thinking for NightHawk over the Anthropic transport through the pair trait only', async () => {
    const provider = registry.createChatProvider({
      protocol: 'anthropic',
      providerType: 'nighthawk',
      modelName: 'nighthawk',
...
describe('reasoning-only assistant history projection', () => {
  ...
  it('keeps unsigned thinking on the NightHawk Anthropic wire', async () => {
    const provider = registry.createChatProvider({
      protocol: 'anthropic',
      providerType: 'nighthawk',
      modelName: 'nighthawk',
...
describe('quota-exhausted classification through the real composition (behavior probes)', () => {
  ...
  it('fails fast on a NightHawk quota 429 over the (nighthawk, anthropic) composition', async () => {
    const provider = registry.createChatProvider({
      protocol: 'anthropic',
      providerType: 'nighthawk',
      modelName: 'nighthawk',
...
describe('reasoning dialect (behavior probes)', () => {
  ...
  it('nighthawk composition defaults to reasoning_content before any detection', async () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'nighthawk',
      modelName: 'nighthawk-k2',
...
describe('responseFormat wire encoding (per base)', () => {
  ...
  it('keeps response_format intact through the NightHawk buildParams pipeline', async () => {
    const provider = registry.createChatProvider({
      protocol: 'openai',
      providerType: 'nighthawk',
      modelName: 'nighthawk-k2',
