...
import '#/kosong/provider/providers/nighthawk/nighthawk.contrib';
...
const HOST_HEADERS = { 'User-Agent': 'nighthawk-test/1.0', 'X-Msh-Device-Id': 'device-1' };
...
beforeEach(() => {
  savedCustomHeaders = process.env['NIGHTHAWK_CUSTOM_HEADERS'];
  delete process.env['NIGHTHAWK_CUSTOM_HEADERS'];
});

afterEach(() => {
  if (savedCustomHeaders === undefined) delete process.env['NIGHTHAWK_CUSTOM_HEADERS'];
  else process.env['NIGHTHAWK_CUSTOM_HEADERS'] = savedCustomHeaders;
});

describe('Model assembly (pure data)', () => {
  it('assembles a nighthawk model: protocol resolves to the vendor base, never a vendor', () => {
    const { host, catalog } = createHost(nighthawkSections);
    try {
  ...
  it('forwards only the User-Agent to vendors without a full hostHeaders declaration', () => {
    ...
    try {
      ...
      expect(model.headers).toEqual({ 'User-Agent': 'nighthawk-test/1.0' });
    } finally {
      host.dispose();
    }
  });
  ...
  describe('custom identity', () => {
    ...
    const OFFICIAL = {
      providers: { nighthawk: { type: 'nighthawk', apiKey: 'sk', baseUrl: 'https://api.example.test/v1' } },
      models: { k2: { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 200000 } },
    };
    ...
    it('preserves a parenthesized User-Agent suffix while rewriting', () => {
      const { host, catalog } = createHost(THIRD_PARTY, stubModelOAuthTokens(), {
        headers: { 'User-Agent': 'nighthawk-test/1.0 (web)' },
        identitySlug: 'acme-dev',
      });
      try {
    ...
    it('changes nothing when no identity is configured', () => {
      ...
      try {
        expect(catalog.get('gpt').headers).toEqual({ 'User-Agent': 'nighthawk-test/1.0' });
      } finally {
        host.dispose();
      }
    });
    ...
    it('attributes the User-Agent provenance for a lowercase host spelling', () => {
      const { host, catalog } = createHost(THIRD_PARTY, stubModelOAuthTokens(), {
        headers: { 'user-agent': 'nighthawk-test/1.0' },
        identitySlug: 'acme-dev',
      });
      try {
        expect(catalog.get('gpt').headers).toEqual({ 'User-Agent': 'acme-dev/1.0' });
  ...
  it('keeps an explicit foreign protocol for a nighthawk model (the dialect path)', () => {
    const { host, catalog } = createHost({
      providers: { nighthawk: { type: 'nighthawk', apiKey: 'sk', baseUrl: 'https://api.example.test/v1' } },
      models: {
        k2: { provider: 'nighthawk', protocol: 'anthropic', model: 'kimi-k2', maxContextSize: 200000 },
      },
    });
    try {
  ...
  it('resolves provider env-bag credentials and endpoints through the registry', () => {
    const { host, catalog } = createHost({
      providers: {
        nighthawk: { type: 'nighthawk', env: { NIGHTHAWK_API_KEY: 'env-token', NIGHTHAWK_BASE_URL: 'https://nighthawk-env.example.test/v1' } },
        openai: { type: 'openai', env: { OPENAI_API_KEY: 'sk-openai' } },
      },
      models: {
        k1: { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 1000 },
        gpt: { provider: 'openai', protocol: 'openai', model: 'gpt-5', maxContextSize: 1000 },
      },
    });
    try {
      const nighthawk = catalog.get('k1');
      expect(nighthawk.baseUrl).toBe('https://nighthawk-env.example.test/v1');
      return expect(nighthawk.authProvider.getAuth()).resolves.toEqual({ apiKey: 'env-token' });
    } finally {
      host.dispose();
    }
  });
  ...
  it('falls back to defaultProvider when a model names no provider', () => {
    const { host, catalog } = createHost({
      ...nighthawkSections,
      defaultProvider: 'nighthawk',
      models: { inherited: { model: 'nighthawk-k2', maxContextSize: 1000 } },
    });
    try {
      expect(catalog.get('inherited').providerName).toBe('nighthawk');
    } finally {
      host.dispose();
    }
  });
  ...
  it('throws config.invalid for unknown models, missing providers, and incomplete records', () => {
    ...
    expectInvalid(nighthawkSections, 'nope');
    ...
    expectInvalid(
      { ...nighthawkSections, models: { noctx: { provider: 'nighthawk', model: 'm' } } },
      'noctx',
    );
  });
  ...
  it('findByName matches name, model, and aliases', () => {
    const { host, catalog } = createHost({
      ...nighthawkSections,
      models: {
        k1: { provider: 'nighthawk', model: 'kimi-k2', aliases: ['k2-latest'], maxContextSize: 1 },
        k2: { provider: 'nighthawk', name: 'shared-name', maxContextSize: 1 },
        k3: { provider: 'nighthawk', model: 'shared-name', maxContextSize: 1 },
      },
    });
    try {
...
describe('ModelCatalog caching and config-event invalidation', () => {
  ...
  it('drops the cache when a watched config section changes', async () => {
    const { host, catalog, models, providers } = createHost(nighthawkSections);
    try {
      const before = catalog.get('k1');
      await models.set('k1', { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 262144, displayName: 'K2' });
      const after = catalog.get('k1');
      expect(after).not.toBe(before);
      expect(after.displayName).toBe('K2');

      await providers.set('nighthawk', { type: 'nighthawk', apiKey: 'sk-2', baseUrl: 'https://other.example.test/v1' });
  ...
  it('keeps serving the stale Model on a silent registry write until notifyConfigChanged()', async () => {
    const { host, catalog, models } = createHost(nighthawkSections);
    try {
      const before = catalog.get('k1');

      silentModelWrite(models, {
        k1: { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 262144, displayName: 'silent' },
      });
...
describe('ModelCatalog inspect', () => {
  it('builds the god object with per-field provenance (nighthawk structured model)', () => {
    const { host, catalog } = createHost(nighthawkSections);
    try {
      const view = catalog.inspect('k1');
      expect(view.id).toBe('k1');
      expect(view.provider).toMatchObject({ id: 'nighthawk', synthesized: false });
  ...
  it('attributes override fields to the overrides block', () => {
    const { host, catalog } = createHost({
      providers: {
        nighthawk: { type: 'nighthawk', apiKey: 'sk', baseUrl: 'https://api.example.test/v1' },
      },
      models: {
        m: {
          provider: 'nighthawk',
  ...
  it('attributes the input cap to config, its clamp, and its absence', () => {
    const { host, catalog } = createHost({
      ...
      models: {
        ...
        plain: { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 100 },
      },
    });
    try {
  ...
  it('attributes env-bag credentials and endpoints by env-var name', () => {
    const { host, catalog } = createHost({
      providers: {
        nighthawk: {
          type: 'nighthawk',
          env: { NIGHTHAWK_API_KEY: 'env-token', NIGHTHAWK_BASE_URL: 'https://nighthawk-env.example.test/v1' },
        },
      },
      models: { k1: { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 1000 } },
    });
    try {
      const view = catalog.inspect('k1');
      expect(view.resolved.baseUrl).toBe('https://nighthawk-env.example.test/v1');
      expect(view.resolved.auth.kind).toBe('apiKey');
      expect(view.sources['resolved.auth']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('NIGHTHAWK_API_KEY'),
      });
      expect(view.sources['resolved.baseUrl']).toMatchObject({
        kind: 'env',
        detail: expect.stringContaining('NIGHTHAWK_BASE_URL'),
      });
  ...
  it('attributes the definition defaultBaseUrl to builtin and reports missing credentials', () => {
    const { host, catalog } = createHost({
      providers: { nighthawk: { type: 'nighthawk' } },
      models: { k1: { provider: 'nighthawk', model: 'kimi-k2', maxContextSize: 1 } },
    });
    try {
      const view = catalog.inspect('k1');
...
describe('ModelCatalog ping', () => {
  it('returns the streamed text and usage on a live success', async () => {
    const { host, models, providers } = createHost(nighthawkSections, stubModelOAuthTokens());
    try {
...
const catalogSections: Record<string, unknown> = {
  providers: {
    nighthawk: { type: 'nighthawk', apiKey: 'sk-test', baseUrl: 'https://api.example.test/v1' },
    openai: { type: 'openai' },
  },
  models: {
    k2: {
    ...
    turbo: { provider: 'nighthawk', model: 'nighthawk-turbo', maxContextSize: 32768, displayName: 'NightHawk Turbo' },
...
describe('wire projection (pure)', () => {
  it('toProtocolModel projects the materialized Model into the snake_case wire shape', () => {
    ...
    try {
      ...
      expect(toProtocolModel(catalog.get('k2'), record, 'nighthawk')).toEqual({
        provider: 'nighthawk',
        model: 'k2',
        display_name: 'NightHawk K2',
  ...
  it('toProtocolModelFallback projects the raw record', () => {
    ...
    expect(toProtocolModelFallback('k2', record, 'nighthawk')).toEqual({
      provider: 'nighthawk',
      model: 'k2',
      display_name: 'NightHawk K2',
      max_context_size: 131072,
...
describe('ModelCatalog enumeration', () => {
  ...
  it('does not project fallback efforts for unknown NightHawk-managed Anthropic models', async () => {
    const sections = structuredClone(catalogSections);
    (sections['models'] as Record<string, ModelRecord>)['compatible'] = {
      provider: 'nighthawk',
      protocol: 'anthropic',
    ...
    try {
      const compatible = (await catalog.listModels()).find((model) => model.model === 'compatible');
      expect(compatible).toMatchObject({ provider: 'nighthawk', model: 'compatible' });
      expect(compatible?.capabilities).toBeUndefined();
      expect(compatible?.support_efforts).toBeUndefined();
      expect(compatible?.default_effort).toBeUndefined();
    } finally {
  ...
  it('detects env-bag credentials through the vendor endpoint declarations', async () => {
    const { host, catalog } = createHost({
      providers: {
        nighthawk: { type: 'nighthawk', env: { NIGHTHAWK_API_KEY: 'nighthawk-env-key' } },
    ...
    try {
      ...
      expect(byId['nighthawk']).toMatchObject({ has_api_key: true, status: 'connected' });
  ...
  it('marks an OAuth provider connected when a cached token exists', async () => {
    const { host, catalog } = createHost(
      {
        providers: { acme: { type: 'nighthawk', oauth: { storage: 'file', key: 'oauth/acme' } } },
        models: {},
      },
  ...
  it('gets a single provider by id and reports provider.not_found for an unknown one', async () => {
    const { host, catalog } = createHost(catalogSections);
    try {
      await expect(catalog.getProvider('nighthawk')).resolves.toMatchObject({
        id: 'nighthawk',
        default_model: 'k2',
        models: ['k2', 'turbo'],
      });
