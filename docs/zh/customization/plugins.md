...
Plugins 把可复用的 NightHawk CLI 能力打包成可安装单元——可以添加 [Agent Skills](./skills.md)、自定义 [Agent](./agents.md)、在会话启动时自动加载指定 Skill、提供系统提示词指令，也可以声明 MCP servers 来提供真实工具能力。适合把工作流共享给团队、连接外部服务，或从[官方插件](#官方插件)安装扩展。
...
- **Installed**：管理已装的
- **Official**：NightHawk 官方 marketplace plugin
- **Curated**：默认 marketplace 中来自 NightHawk 合作伙伴的第三方 plugin
- **Custom**：从 URL 安装
...
- 本地安装会被拷贝到 `$NIGHTHAWK_HOME/plugins/managed/<id>/`，CLI 始终从这份托管副本运行。安装后编辑原始源目录不会生效，需重新安装。
- 移除 plugin 只会删除安装记录，托管副本和原始源文件仍保留在磁盘上。
- Plugin 目前按用户安装，对所有项目生效，暂不支持项目级安装范围。

### 自定义 marketplace JSON

浏览自定义目录时，把 JSON 路径或 URL 传给 `/plugins marketplace <source>`；或通过 [`NIGHTHAWK_PLUGIN_MARKETPLACE_URL`](../configuration/env-vars.md) 覆盖默认 marketplace。`plugins` 数组中每个条目需要 `id` 和 `source`（本地路径、zip URL 或 GitHub URL）：
...
官方插件是 NightHawk 官方维护的 plugin 和内置产品能力，目前有以下三种：
...
::: info 说明
NightHawk WebBridge 分两步安装：完成上述步骤后，还需要[安装浏览器扩展](#install-the-browser-extension)才能使用。
:::
...
NightHawk Datasource 是 NightHawk 官方数据插件，让你用自然语言直接查询金融行情、财经资讯、宏观经济、企业工商、学术文献、中国法律法规和国际组织官方数据，无需手动调用接口或申请数据账号。
...
1. 直接用自然语言描述你的需求，NightHawk 会自动调用数据能力
2. 通过 `/skill:nighthawk-datasource` 明确触发数据查询 Skill
...
NightHawk WebBridge 让 AI 直接操控你的浏览器，带着你的登录状态和 Cookie，AI 可以像你一样打开网页、阅读内容、点击按钮、填写表单、截图保存，把重复繁琐的网页操作交给它完成。产品介绍见你的 NightHawk 分发版中的 WebBridge 页面。

<a id="install-the-browser-extension"></a>

#### 安装浏览器扩展

通过 `/plugins` 安装后，还需要在浏览器中安装 NightHawk WebBridge 扩展，AI 才能操控你的浏览器。有两种安装方式：

**方式一：应用商店安装（推荐）**

打开 [Chrome 应用商店](https://chromewebstore.google.com/detail/nighthawk-webbridge/fldmhceldgbpfpkbgopacenieobmligc)或 [Edge 应用商店](https://microsoftedge.microsoft.com/addons/detail/nighthawk-webbridge/bnlffdbcfnanfbknnlaflhlhkocccckg)，点击添加即可。
...
两个文件同时存在时，以 `nighthawk.plugin.json` 为准。
...
manifest（`nighthawk.plugin.json`）用 `commands` 字段指出命令文件的位置：
...
NightHawk 会把正文里的 `$ARGUMENTS` 替换成 `TSLA`，再执行这段提示词。三处细节分述如下。
...
命令自动以插件 id 作前缀（即命名空间），注册成 `<插件名>:<命令名>`，所以上面的命令实际叫 `/nighthawk-finance:report`，不同插件的同名命令因此不会冲突。
...
`sessionStart.skill` 在会话启动时把一个 plugin Skill 加载到 main agent，适合放置初始化说明、工作流规则，或把其他工具中的术语映射到 NightHawk CLI。它只注入文本，不执行代码。
...
- hook 进程会额外收到两个环境变量：`NIGHTHAWK_HOME` 和 `NIGHTHAWK_PLUGIN_ROOT`（plugin 根目录）。
