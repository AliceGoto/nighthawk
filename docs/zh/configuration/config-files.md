...
CLI 从 `~/.nighthawk/config.toml` 读取配置。如需把数据目录迁移到别处，可用 `NIGHTHAWK_HOME` 环境变量覆盖：

```sh
export NIGHTHAWK_HOME=/path/to/nighthawk-home
```

此时配置文件路径变为 `$NIGHTHAWK_HOME/config.toml`。无论目录在哪里，文件名固定是 `config.toml`。
...
| `builtin_product_skills` | `boolean` | `true` | 是否向模型提供介绍 NightHawk 自身的内置 Skills：`update-config`、`custom-theme`、`mcp-config`、`check-nighthawk-docs`、`import-from-cc-codex`。关闭后它们的名称和描述不再进入系统提示词，代价是失去这些任务的引导流程。默认的 `agent-core-v2` 引擎会读取本字段；设置 `NIGHTHAWK_LEGACY_FLAG=1` 选择旧版引擎时会忽略 |
...
| `support_efforts` | `array<string>` | 否 | 模型接受的 Thinking 档位。对 `nighthawk` 而言，在运行时选择列表外的值会报错；模型解析时若配置值或之前的值不受目标模型支持，会回落到目标模型的 `default_effort`，并将该有效值同步给 UI。支持 Thinking 但没有此字段的 NightHawk 模型使用布尔 `on` / `off`。其他 provider 在协议提供原生 effort 字段时会原样传递具体值；协议仅提供等级或 token budget 时，只做必要的格式转换。managed 和 open-platform 刷新可能会改写该字段；如需手动固定，请改用 `[models."<alias>".overrides] support_efforts` |
...
无需修改配置文件也可以临时切换模型——通过 `NIGHTHAWK_MODEL_*` 环境变量在内存里合成一个临时供应商，详见[用环境变量定义模型](./env-vars.md#用环境变量定义模型-nighthawk-model)。
...
该功能目前是实验功能，默认关闭。通过 `NIGHTHAWK_EXPERIMENTAL_SECONDARY_MODEL=1` 启用，或使用 master `NIGHTHAWK_EXPERIMENTAL_FLAG=1`，在包括交互式 TUI 在内的所有启动方式下生效。实验功能关闭时模型池配置不生效：subagent 继承调用方模型，会话启动也会跳过池校验。
...
"nighthawk/k3" = "难题选它。擅长复杂推理、算法设计、深度调试、数学和系统性难题。"
"nighthawk/nighthawk-highspeed" = "速度快但单价较高。适合日常重构、代码解释、小改动、总结等看重响应速度的任务。"
"nighthawk/nighthawk" = "均衡的编码主力。适合大多数功能开发和代码修改任务。"
```
...
default_model = "nighthawk/k3"
[secondary_model.models]
"nighthawk/k3" = "默认 high 档位。适合大多数实现、分析和多轮交互任务。"
k3-max = "同一模型的 max Thinking 档位。适合最难的子任务。"
```

两个前提：

- 底层模型必须声明了 `support_efforts`（`managed:nighthawk` 下目前只有 k3 系列声明了档位）。
...
| `effort` | `string` | — | Thinking 强度（例如 `low`、`medium`、`high`、`xhigh`、`max`）。非 NightHawk provider 在上游协议接受具体 effort 值时不会改写该值；如果上游拒绝，请改成该模型支持的档位。协议仅提供等级或 token budget 时，仍需做格式转换。对于带 `support_efforts` 的 NightHawk 模型，若该配置值不在列表中，会回落到模型默认档位；没有该列表的 NightHawk 模型会把任意开启值视为布尔 `on` |
| `keep` | `string` | `"all"` | 保留思考透传。在 `nighthawk` 上以 `thinking.keep` 发送；在 `anthropic`（Claude 以及 NightHawk 的 Anthropic 兼容模式）上以 `context_management` 的 `clear_thinking_20251015` 编辑发送（开启 keep 会让 Anthropic 请求走 beta Messages API；关值可禁用 keep 并回到标准端点）。`"all"` 会保留历史轮次的思考内容（`reasoning_content` / Anthropic thinking blocks）；传入关值（`false`/`0`/`no`/`off`/`none`/`null`）可禁用。可被 `NIGHTHAWK_MODEL_THINKING_KEEP` 覆盖；仅在 Thinking 开启时注入 |
...
`strategy` 可被环境变量 `NIGHTHAWK_TOKEN_COUNTING_STRATEGY` 覆盖，优先级高于 `config.toml`。
...
| `keep_alive_on_exit` | `boolean` | `false` | 会话关闭时是否保留仍在运行的后台任务。默认情况下，NightHawk 会在进程退出前请求停止所有后台任务；只有希望任务在会话结束后继续运行时才设为 `true`。在 print 模式（`nighthawk -p`）下，本字段仅作为 `print_background_mode` 未设置时的兼容回退：`true` 等价于 `print_background_mode = "drain"` |
| `kill_grace_period_ms` | `integer` | `5000` | 会话关闭、手动停止或任务超时请求正常终止后，等待任务自行结束的宽限时间（毫秒）。超过该时间仍在运行时，NightHawk 会尝试强制停止该任务 |
...
| `bash_task_timeout_s` | `integer` | `600` | 后台 `Bash` 任务在调用未传 `timeout` 时的默认超时（秒）；前台命令超时转后台后也按此值重新计时。`0` 表示无超时——任务一直运行到自行结束或被模型手动停止。显式传入的 `timeout` 不受影响。在 print 模式（`nighthawk -p`）下未显式设置时默认为 `0` |
| `print_background_mode` | `"exit" \| "drain" \| "steer"` | `"steer"` | 仅 print 模式（`nighthawk -p`）生效，决定 main agent 的 turn 结束后如何处理未返回的后台任务：`"exit"` 立即退出；`"drain"` 退出前等待所有后台任务进入终态（结果不回馈给 main agent）；`"steer"` 不退出，让后台任务完成时像后台 subagent 一样以合成 user 消息 steer main agent 进入新 turn，直到某 turn 结束时无未决后台任务或触及上限。设置后优先级高于 `keep_alive_on_exit` 的 print 回退 |
| `print_wait_ceiling_s` | `integer` | `2147483` | print 模式（`nighthawk -p`）下，`print_background_mode` 为 `"drain"` 或 `"steer"` 时，等待/steer 循环的墙钟上限（秒；默认约 24.8 天，近似不设限）。在非 print 模式或 `"exit"` 时无效 |
| `print_max_turns` | `integer` | `100000` | print 模式（`nighthawk -p`）且 `print_background_mode = "steer"` 时，允许由后台任务完成触发的新 turn 的最大数量，防止 steer 循环失控（默认值近似不设限） |

`keep_alive_on_exit` 可被环境变量 `NIGHTHAWK_BACKGROUND_KEEP_ALIVE_ON_EXIT` 覆盖，`max_running_tasks` 可被 `NIGHTHAWK_BACKGROUND_MAX_RUNNING_TASKS` 覆盖，优先级均高于配置文件。

在 print 模式（`nighthawk -p "<prompt>"`）下，只要还有未决的后台任务，NightHawk 在 main agent 的 turn 结束后不会退出：每个任务完成都会以合成 user 消息回馈给 main agent，steer 出新的 turn（默认 `print_background_mode = "steer"`），直到某 turn 结束时没有任何未决任务才退出。该循环受 `print_wait_ceiling_s` 与 `print_max_turns` 约束，默认值都近似不设限。print 模式下后台工作也不会被墙钟超时杀掉：后台 `Bash` 任务默认无超时（`bash_task_timeout_s = 0`），subagent 默认无超时（`[subagent] timeout_ms` 与 `[swarm] timeout_ms` 未显式设置时均为 `0`），只有模型自己能停止任务。将 `print_background_mode` 设为 `"drain"` 可等待任务结束但不回馈结果，设为 `"exit"` 则在 main agent 结束后立即退出。
...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `timeout_ms` | `integer` | `7200000`（2 小时） | 单个 `Agent` subagent 允许运行的最长时间（毫秒）。超时后 subagent 以 `timed_out` 收尾。`0` 表示无超时——subagent 一直运行到自行结束或被模型手动停止。该值是后台任务管理器对每个 subagent 任务的 per-task timeout，因此对前台与后台 subagent 同时生效。在 print 模式（`nighthawk -p`）下未显式设置时默认为 `0`。注意：超过 `2147483647`（约 24.8 天）的值会被运行时钳到约 24.8 天 |

`timeout_ms` 可被环境变量 `NIGHTHAWK_SUBAGENT_TIMEOUT_MS` 覆盖，优先级高于配置文件。
...
| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `timeout_ms` | `integer` | `7200000`（2 小时） | `AgentSwarm` 启动的单个 subagent 允许运行的最长时间（毫秒）。超时后该 subagent 被中止，聚合报告中标记为失败（`Subagent timed out.`），其余 subagent 不受影响。`0` 表示无超时——subagent 一直运行到自行结束或被模型手动停止。在 print 模式（`nighthawk -p`）下未显式设置时默认为 `0`。注意：超过 `2147483647`（约 24.8 天）的值会被运行时钳到约 24.8 天 |

`timeout_ms` 可被环境变量 `NIGHTHAWK_SWARM_TIMEOUT_MS` 覆盖，优先级高于配置文件。
...
`startup_timeout_ms` 和 `tool_timeout_ms` 可分别被环境变量 `NIGHTHAWK_MCP_STARTUP_TIMEOUT_MS` 和 `NIGHTHAWK_MCP_TOOL_TIMEOUT_MS` 覆盖，优先级高于配置文件。MCP server 的完整配置方式见 [MCP](../customization/mcp.md)。
...
两个字段都可以通过 `NIGHTHAWK_IDENTITY_NAME` 和 `NIGHTHAWK_IDENTITY_SLUG` 环境变量设置，优先级高于 `config.toml`，且不会被写回配置文件——适合不便写配置文件的容器和 CI 场景。
...
`max_edge_px` 可被环境变量 `NIGHTHAWK_IMAGE_MAX_EDGE_PX` 覆盖，`read_byte_budget` 可被 `NIGHTHAWK_IMAGE_READ_BYTE_BUDGET` 覆盖，优先级均高于配置文件。
...
`services` 配置网页搜索（`nighthawk_search`）和网页抓取（`nighthawk_fetch`）两项内置服务。只识别这两个固定 key，其他 key 会被忽略。两项字段相同：
...
::: tip
MCP server 的声明配置写在 `~/.nighthawk/mcp.json` 或项目内 `.nighthawk/mcp.json` 中，不在 `config.toml` 里。交互式配置入口是 `/mcp-config`，详见 [Model Context Protocol](../customization/mcp.md)。
:::

## `tui.toml`

除了 `config.toml`，CLI 还在同一目录下用一份配套的 `tui.toml` 保存终端界面与客户端偏好（`~/.nighthawk/tui.toml`，或覆盖后的 `$NIGHTHAWK_HOME/tui.toml`）。它在首次运行时以默认值创建，交互式命令 `/config`、`/theme`、`/editor` 会自动写入，通常无需手动编辑。文件格式有误时，CLI 会回退到默认值并给出提示，而不是启动失败。
...
除了 `~/.nighthawk` 下的用户级文件，NightHawk 还会读取位于 `<项目根目录>/.nighthawk/local.toml` 的项目级本地配置文件。它保存的是与某一个项目检出相关、通常不应与队友共享的设置。
...
目录以绝对路径存储，与具体机器相关。因此建议把 `.nighthawk/local.toml` 加入项目的 `.gitignore`，避免被提交。
