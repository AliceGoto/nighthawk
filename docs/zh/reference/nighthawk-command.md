...
`nighthawk` 是 NightHawk CLI 的主命令，用于在终端中启动一次交互式会话。不带任何参数运行时，它会在当前工作目录下开启一个新会话；配合不同的 flag，可以续上历史会话、跳过审批、从 Plan 模式开始，或者指定自定义的 Skills 目录。
...
所有 flag 都是可选的，直接运行 `nighthawk` 即可进入交互式会话：
...
恢复会话时，可以通过 `--auto`、`--yolo` 或 `--plan` 覆盖原会话保存的权限或计划模式。例如，`nighthawk --continue --auto` 会恢复最近会话并切换到 auto 权限模式。
...
`--agent` 和 `--agent-file` 用于选择驱动新会话的 Agent，在 print 模式（`nighthawk -p`）和交互式 TUI 中均可使用：

```sh
nighthawk --agent reviewer
nighthawk -p --agent reviewer "审查这个分支上的改动"
```
...
把 NightHawk CLI 切换到 ACP（Agent Client Protocol）模式，在标准输入/输出上以 JSON-RPC 形式与 IDE 对话，让编辑器直接驱动 NightHawk 的会话和工具调用。通常不需要手动运行——IDE 会把它作为子进程入口启动。配置方式见[在 IDE 中使用](../guides/ides.md)，技术细节见 [nighthawk acp 参考](./nighthawk-acp.md)。
...
校验 `config.toml` 和 `tui.toml`，不会启动 TUI，也不会修改任一文件。默认检查 `NIGHTHAWK_HOME` 下的文件；未设置该环境变量时检查 `~/.nighthawk`。默认路径缺失时会显示为跳过，因为内置默认值仍可生效。
...
| 命令 | 说明 |
| --- | --- |
| `nighthawk doctor` | 校验默认 `config.toml` 和 `tui.toml` |
| `nighthawk doctor config [path]` | 只校验 `config.toml`；传入 `path` 时使用该文件而不是默认文件 |
| `nighthawk doctor tui [path]` | 只校验 `tui.toml`；传入 `path` 时使用该文件而不是默认文件 |
...
导出包含目标会话目录内的所有文件。全局诊断日志（`~/.nighthawk/logs/nighthawk.log`）默认包含，因为它可能含有其他会话或项目的事件；不想分享时加 `--no-include-global-log`。
...
将旧版 legacy-cli 的本地数据迁移到 nighthawk，包括历史会话和配置文件。纯交互式运行，会引导你完成全流程。

```sh
nighthawk migrate
```

完整迁移说明见[从 legacy-cli 迁移](../guides/migration.md)。

### `nighthawk upgrade`

立即检查最新版本并展示更新提示，选择操作后退出。也可以使用别名 `nighthawk update`。

```sh
nighthawk upgrade
```

对全局 npm、pnpm、yarn、bun 安装，`nighthawk upgrade` 会展示更新选项；选择 `Install update now` 后运行对应的前台安装命令。对 native 安装（含 Windows），会在前台下载并校验新二进制，并在下次启动时替换生效。当前安装方式无法自动升级时，改为打印手动更新命令。
...
| --- | --- |
| `<url>` | Registry 地址 |
| `--api-key <key>` | 访问 registry 时携带的 Bearer token。未传时回退到环境变量 `NIGHTHAWK_REGISTRY_API_KEY`，必填 |
...
| `--api-key <key>` | 供应商 API key。未传时回退到 `NIGHTHAWK_REGISTRY_API_KEY`，必填 |
...
```sh
nighthawk provider catalog list anthropic          # 先看可选的模型
