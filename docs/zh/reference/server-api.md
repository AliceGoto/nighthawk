...
默认地址为 `http://127.0.0.1:58627`。端口被占用时，服务会用下一个端口重试（至多 100 次）；可用 `--port` / `--host` 修改绑定。同一 home 目录下可并存多个实例，运行中的实例登记在 `~/.nighthawk/server/instances/`。
...
携带方式：REST 用 `Authorization: Bearer <token>` 请求头；WebSocket 升级请求接受同一请求头，或子协议 `nighthawk.bearer.<token>`。token 的生成与轮换见 [本地服务与 API：鉴权](../guides/server.md#authentication)。
...
这组端点管理模型配置的两半——`config.toml` 的 [供应商](../configuration/providers.md) 表与模型别名表——外加一个由服务端代理的 models.dev 目录，用于一次性导入。模型别名 id 就是配置中的别名键：通过供应商管理端点创建的别名形如 `provider_id/model`（例如 `my-provider/nighthawk`），而模型别名表中的裸键（如 `turbo`）原样使用；API 中任何接收 `model_id` 的地方（包括全局 `default_model`）指的都是这个别名 id。`:{action}` 路由上不支持的动作返回 `40001`。
...
把全局 `default_model` 设为一个已存在的别名。`model_id` 是配置中的别名键原样——裸键如 `POST /api/v1/models/turbo:set_default`；当 id 含 `/` 时需做 URL 编码，如 `POST /api/v1/models/my-provider%2Fnighthawk:set_default`。
...
| --- | --- | --- |
| `id` | string | 供应商 id |
| `type` | string | 通信协议：`nighthawk` / `openai` / `openai_responses` / `anthropic` / `google-genai` / `vertexai` |
...
| `type` | body | string | **必填。** 通信协议：`nighthawk` / `openai` / `openai_responses` / `anthropic` / `google-genai` / `vertexai` |
...
将会话连同诊断日志一起导出为 zip 附件（`nighthawk-session-<id>.zip`）。响应是二进制流，不是 JSON 信封——能力与失败语义见 [二进制与流式端点](#二进制与流式端点)。
...
能力是带有分层就绪状态的内置特性——由检测步骤加后台安装组成；当前版本注册了 `nighthawk-cu`（NightHawk Computer Use）与 `nighthawk-webbridge`（NightHawk WebBridge）。插件是已安装的技能、MCP 服务、hook 与命令的打包集合。这组端点报告能力状态、驱动能力安装，并管理插件从市场列表到移除的整个生命周期。
