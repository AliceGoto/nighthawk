...
- **Official**: NightHawk-maintained marketplace plugins
- **Curated**: Third-party plugins from NightHawk partners in the default marketplace
...
- Local installations are copied to `$NIGHTHAWK_HOME/plugins/managed/<id>/`, and the CLI always runs from this managed copy. Editing the original source directory after installation has no effect; you must reinstall.
...
Pass a custom marketplace JSON path or URL to `/plugins marketplace <source>`, or set [`NIGHTHAWK_PLUGIN_MARKETPLACE_URL`](../configuration/env-vars.md) to override the default catalog. Each entry in the `plugins` array needs an `id` and a `source` (local path, zip URL, or GitHub URL):
...
Official plugins are plugins and built-in product capabilities maintained by NightHawk. There are currently three:
...
::: info Note
NightHawk WebBridge installs in two parts: after the steps above, you also need to [install the browser extension](#install-the-browser-extension) before it works.
:::
...
### NightHawk Datasource <Badge type="tip" text="v3.4.0" />

NightHawk Datasource is the official NightHawk data plugin, letting you query financial market data, financial news, macroeconomic indicators, corporate registration records, academic literature, Chinese laws and regulations, and official data from intergovernmental organizations in natural language — no manual API calls or data accounts required.
...
1. Describe your need in natural language, and NightHawk will automatically invoke the data capabilities
2. Explicitly trigger the data query skill with `/skill:nighthawk-datasource`
...
- Data queries are billed per call and consume NightHawk account credits
...
### NightHawk WebBridge <Badge type="tip" text="v1.11.3" />

NightHawk WebBridge lets AI drive your browser directly — not an emulator, not a crawler, but the browser you use every day, with your login sessions and cookies. AI can open pages, read content, click buttons, fill in forms, and take screenshots just like you do, taking repetitive web operations off your hands. See the WebBridge page of your NightHawk distribution for a product overview.

#### Install the browser extension

After installing via `/plugins`, you also need the NightHawk WebBridge extension in your browser before AI can drive it. There are two ways to install it:

**Option 1: Install from a store (recommended)**

Open the [Chrome Web Store](https://chromewebstore.google.com/detail/nighthawk-webbridge/fldmhceldgbpfpkbgopacenieobmligc) or [Edge Add-ons](https://microsoftedge.microsoft.com/addons/detail/nighthawk-webbridge/bnlffdbcfnanfbknnlaflhlhkocccckg) page and click Add.
...
```text
<plugin_root>/nighthawk.plugin.json
<plugin_root>/.nighthawk-plugin/plugin.json
```

When both files exist, `nighthawk.plugin.json` takes precedence.

Example:

```json
{
  "name": "nighthawk-finance",
  "version": "1.0.0",
  "description": "Finance data and analysis workflows for NightHawk CLI",
  "skills": "./skills/",
...
In the manifest (`nighthawk.plugin.json`), the `commands` field points to where the command files live:
...
```text
/nighthawk-finance:report TSLA
```

NightHawk replaces `$ARGUMENTS` in the body with `TSLA`, then runs the prompt. The three details below cover each step.
...
Commands are prefixed with the plugin id (their namespace) and registered as `<plugin>:<command>`, so the command above is actually `/nighthawk-finance:report` — this keeps same-named commands from different plugins from colliding.
...
`sessionStart.skill` loads a plugin Skill into the main Agent at session start, making it suitable for initialization instructions, workflow rules, or mapping terminology from other tools to NightHawk CLI. It only injects text; it does not execute code.
...
```sh
/plugins mcp disable nighthawk-finance finance
/reload

/plugins mcp enable nighthawk-finance finance
/reload
```
...
- The hook process receives two extra environment variables: `NIGHTHAWK_HOME` and `NIGHTHAWK_PLUGIN_ROOT` (the plugin root directory).
