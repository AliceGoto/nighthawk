...
The default address is `http://127.0.0.1:58627`. When the port is taken, the server retries with the next port (up to 100 times); use `--port` / `--host` to change the bind. Multiple instances can coexist under the same home directory; running instances register under `~/.nighthawk/server/instances/`.
...
How to carry it: REST uses the `Authorization: Bearer <token>` header; the WebSocket upgrade accepts the same header or the subprotocol `nighthawk.bearer.<token>`. Token generation and rotation are covered in [Local server and API: Authentication](../guides/server.md#authentication).
...
Sets the global `default_model` to an existing alias. `model_id` is the exact configured alias key — for a bare key like `turbo` the call is `POST /api/v1/models/turbo:set_default`; URL-encode the id when it contains `/`, as in `POST /api/v1/models/my-provider%2Fnighthawk:set_default`.
...
| `type` | string | Wire protocol: `nighthawk` / `openai` / `openai_responses` / `anthropic` / `google-genai` / `vertexai` |
...
| `type` | body | string | **Required.** Wire protocol: `nighthawk` / `openai` / `openai_responses` / `anthropic` / `google-genai` / `vertexai` |
...
Exports the session together with diagnostic logs as a zip attachment (`nighthawk-session-<id>.zip`). The response is a binary stream, not a JSON envelope — capabilities and failure semantics are covered under [Binary and streaming endpoints](#binary-and-streaming-endpoints).
...
Capabilities are built-in features with layered readiness — detection steps plus a background install; the current build registers `nighthawk-cu` (NightHawk Computer Use) and `nighthawk-webbridge` (NightHawk WebBridge). Plugins are installed packages of skills, MCP servers, hooks, and commands. These endpoints report capability status and drive capability installs, and manage the plugin lifecycle from marketplace listing to removal.
