...
editor and NightHawk.

Version `0.6.0` moves the extension into this monorepo under `apps/vscode` and
runs the stable TypeScript v1 engine through `@nighthawk/nighthawk-sdk` in the
...
- Keep the extension ID `nighthawk.nighthawk` so `0.6.0` upgrades existing
  installations.
...
- Share NightHawk configuration, authentication, MCP configuration, and
  sessions with the TUI when both processes resolve the same NightHawk home.
...
flowchart LR
  ...
  SDK["@nighthawk/nighthawk-sdk<br/>NighthawkHarness and Session"]
  Core["v1 agent-core"]
  Home["NightHawk home<br/>config, auth, MCP, sessions"]
...
- `productName: "nighthawk-vscode"`
- `version` from `apps/vscode/package.json`
- `uiMode: "vscode"`

For `0.6.0`, the normal HTTP User-Agent product is therefore
`nighthawk-vscode/0.6.0`. The version has one source of truth and is not copied
into runtime code or packaging scripts.

### Package boundaries

- `apps/vscode` depends on `@nighthawk/nighthawk-sdk`.
- `apps/vscode` must not depend directly on `@nighthawk/agent-core`.
...
| Webview lifecycle | `apps/vscode/src/NighthawkWebviewProvider.ts` |
| Webview RPC boundary | `apps/vscode/src/bridge-handler.ts`, `apps/vscode/src/handlers` |
| SDK host | `apps/vscode/src/runtime/nighthawk-runtime.ts` |
...
### Shared NightHawk home

The SDK resolves the home directory using the normal NightHawk rules:

1. system-level `NIGHTHAWK_HOME`, when set;
2. otherwise `~/.nighthawk`.
...
- `sessions/`
- `session_index.jsonl`
- other SDK-owned NightHawk data
...
- proxy and other process-level variables are inherited from the Extension
  Host environment.
- a legacy `NIGHTHAWK_SHARE_DIR` in the removed setting is consulted only as an
...
Migration is opt-in and uses `@nighthawk/migration-legacy` for detection and
...
- default source: `~/.kimi`;
- optional additional source: a valid legacy `NIGHTHAWK_SHARE_DIR` from the removed
  VS Code setting;
- target: the SDK-resolved NightHawk home.
...
`NightHawk: Migrate Legacy Data` remains available for manual runs and retries.

The shared marker `.migrated-to-nighthawk` can contain multiple target homes.
This prevents duplicate migration when the TUI migrated the same source first,
while still allowing a different `NIGHTHAWK_HOME` to be migrated later.
...
resolved NightHawk home and session ID. The home namespace prevents sessions
...
3. Shared config and sessions live in the SDK-resolved NightHawk home; editor
