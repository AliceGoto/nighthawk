...
export interface NighthawkTUIStartupInput {
  ...
  /** When true, run only the migration screen, then exit (the `nighthawk migrate` command). */
...
function createInitialAppState(input: NighthawkTUIStartupInput): AppState {
...
export class NighthawkTUI {
  ...
  constructor(harness: NighthawkHarness, startupInput: NighthawkTUIStartupInput) {
    ...
    const tuiOptions: NighthawkTUIOptions = {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Debug-only input→render latency overlay (NIGHTHAWK_TUI_INPUT_LATENCY=1).
      if (process.env['NIGHTHAWK_TUI_INPUT_LATENCY']) installInputLatencyProbe(this.state.ui);
  ...
  private async showResumeOtherWorkDirHint(session: SessionRow): Promise<void> {
    ...
    const command = `cd ${quoteShellArg(session.work_dir)} && nighthawk --resume ${quoteShellArg(session.id)}`;
  ...
  private async runMigrationScreen(plan: MigrationPlan): Promise<MigrationScreenResult> {
    ...
    if (result.decision === 'never') {
      ...
      try {
        writeFileSync(join(this.harness.homeDir, '.skip-migration-from-legacy-cli'), '', 'utf-8');
      } catch {
  ...
  private showApprovalPanel(payload: ApprovalPanelData): void {
    ...
    notifyTerminalOnce(this.state, `approval:${payload.id}`, {
      title: 'NightHawk approval required',
  ...
  private showQuestionDialog(payload: QuestionPanelData): void {
    ...
    notifyTerminalOnce(this.state, `question:${payload.id}`, {
      title: 'NightHawk needs your answer',
