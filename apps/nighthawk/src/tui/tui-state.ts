...
export function createTUIState(options: NighthawkTUIOptions): TUIState {
  ...
  // Fullscreen is experimental and env-gated for now: NIGHTHAWK_TUI_FULL_SCREEN=1.
  const fullscreen = process.env['NIGHTHAWK_TUI_FULL_SCREEN'] === '1';
  const ui =
    fullscreen
