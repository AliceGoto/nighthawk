...
const mocks = vi.hoisted(() => ({
  ...
  resolveNighthawkHome: vi.fn((homeDir?: string) => homeDir ?? '/tmp/nighthawk-export-home'),
...
vi.mock('@nighthawk/nighthawk-sdk', async (importOriginal) => {
  ...
  const createFakeHarness = (options: { readonly homeDir?: string } | undefined) => {
    const homeDir = options?.homeDir ?? '/tmp/nighthawk-export-home';
...
afterEach(() => {
  ...
  mocks.resolveNighthawkHome.mockImplementation(
    (homeDir?: string) => homeDir ?? '/tmp/nighthawk-export-home',
  );
...
describe('nighthawk export', () => {
  ...
  it('parses --no-include-global-log as an option when no session id is given', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'export', '--no-include-global-log', '-y']);
  ...
  it('initializes and flushes telemetry around default export tracking', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'export', 'ses_telemetry', '--output', output], {
      from: 'node',
    });
  ...
  it('passes enabled false when default export config disables telemetry', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'export', 'ses_disabled', '--output', output], {
      from: 'node',
    });
  ...
  it('tracks first launch around default export telemetry before harness construction can create the device id', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'export', 'ses_first_launch', '--output', output], {
      from: 'node',
    });
    ...
    expect(mocks.nighthawkHarnessConstructor).toHaveBeenCalledWith(
      expect.objectContaining({ homeDir: '/tmp/nighthawk-export-home' }),
    );
  ...
  it('builds the v2 harness by default', async () => {
    vi.stubEnv('NIGHTHAWK_LEGACY_FLAG', '');
    const program = new Command('nighthawk');
    ...
    await program.parseAsync(['node', 'nighthawk', 'export', 'ses_v2_engine', '--output', output], {
      from: 'node',
    });
  ...
  it('builds the legacy harness when the legacy flag is truthy', async () => {
    ...
    await program.parseAsync(['node', 'nighthawk', 'export', 'ses_legacy_engine', '--output', output], {
      from: 'node',
    });
