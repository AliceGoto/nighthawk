 ...
 * Run: pnpm -C apps/nighthawk exec vitest run test/cli/options.test.ts
 */
...
function parse(argv: string[]): CLIOptions {
  ...
  program.parse(['node', 'nighthawk', ...argv]);
...
describe('CLI options parsing', () => {
  ...
  describe('--version', () => {
    it('prints the version string and exits', () => {
      ...
      expect(() => program.parse(['node', 'nighthawk', '--version'])).toThrow();
    ...
    it('supports -V as a short alias', () => {
      ...
      expect(() => program.parse(['node', 'nighthawk', '-V'])).toThrow();
  ...
  describe('--model / -m', () => {
    it('parses -m as a model override', () => {
      expect(parse(['-m', 'nighthawk/k2']).model).toBe('nighthawk/k2');
    });

    it('parses --model=value as a model override', () => {
      expect(parse(['--model=nighthawk/k2.5']).model).toBe('nighthawk/k2.5');
    });
  ...
  describe('NIGHTHAWK_MODEL_OUTPUT_FORMAT', () => {
    ...
    it('rejects an invalid env value', () => {
      ...
      ).toThrow('Invalid NIGHTHAWK_MODEL_OUTPUT_FORMAT value "json"');
    });
  ...
  describe('--agent / --agent-file', () => {
    ...
    it('accepts the flags in prompt mode with the legacy engine flag', () => {
      const opts = parse(['-p', 'hi', '--agent', 'reviewer']);
      expect(validateOptions(opts, { NIGHTHAWK_LEGACY_FLAG: '1' }).uiMode).toBe('print');
    });
  });
  ...
  describe('sub-commands', () => {
    it('routes upgrade without calling the main action', () => {
      ...
      program.parse(['node', 'nighthawk', 'upgrade']);
    ...
    it('routes update alias to the upgrade handler', () => {
      ...
      program.parse(['node', 'nighthawk', 'update']);
