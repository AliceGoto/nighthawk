...
import { nighthawkPluginMarketplaceUrl } from '#/constant/app';
...
import { NighthawkTUI, type NighthawkTUIStartupInput, type TUIState } from '#/tui/nighthawk-tui';
...
function makeStartupInput(): NighthawkTUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
      continue: false,
...
function makeHarness(session = makeSession(), overrides: Record<string, unknown> = {}) {
  ...
  const harness = {
    getConfig: vi.fn(async () => ({
      models: {
        k2: { model: 'nighthawk-v1', maxContextSize: 100 },
      },
    })),
    ...
    auth: {
      ...
      status: vi.fn(async () => ({
        providers: [{ providerName: 'managed:nighthawk', hasToken: true }],
      })),
      login: vi.fn(),
      logout: vi.fn(),
...
async function makeDriver(
  ...
  startupInput: NighthawkTUIStartupInput = makeStartupInput(),
): Promise<{
  ...
  const driver = new NighthawkTUI(harness as never, startupInput) as unknown as MessageDriver;
...
const originalNighthawkHome = process.env['NIGHTHAWK_HOME'];
const originalPluginMarketplaceUrl = process.env['NIGHTHAWK_PLUGIN_MARKETPLACE_URL'];
const originalVisual = process.env['VISUAL'];
const originalEditor = process.env['EDITOR'];

async function makeTempHome(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-tui-'));
  tempDirs.push(dir);
  return dir;
}
...
async function makeExportedSessionZip(content = 'session zip'): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'nighthawk-feedback-export-'));
  tempDirs.push(dir);
...
afterEach(async () => {
  ...
  if (originalNighthawkHome === undefined) {
    delete process.env['NIGHTHAWK_HOME'];
  } else {
    process.env['NIGHTHAWK_HOME'] = originalNighthawkHome;
  }
  ...
  if (originalPluginMarketplaceUrl === undefined) {
    delete process.env['NIGHTHAWK_PLUGIN_MARKETPLACE_URL'];
  } else {
    process.env['NIGHTHAWK_PLUGIN_MARKETPLACE_URL'] = originalPluginMarketplaceUrl;
  }
...
describe('NighthawkTUI message flow', () => {
  ...
  it('lazily creates the session on the first message (v2 engine)', async () => {
    ...
    const startupInput: NighthawkTUIStartupInput = {
      ...makeStartupInput(),
      engineV2: true,
  ...
  it('does not pass the config default plan mode into the lazy-created session (v2 engine)', async () => {
    ...
    const { driver, harness } = await makeDriver(
      ...
      {
        getConfig: vi.fn(async () => ({
          models: { k2: { model: 'nighthawk-v1', maxContextSize: 100 } },
          defaultModel: 'k2',
  ...
  it('hydrates lazy config defaults on a sessionless /reload (v2 engine)', async () => {
    ...
    process.env['NIGHTHAWK_HOME'] = homeDir;
    const session = makeSession({ id: 'ses-lazy' });
  ...
  it('tracks theme changes from slash commands', async () => {
    process.env['NIGHTHAWK_HOME'] = await makeTempHome();
    const { driver, harness } = await makeDriver();
    harness.track.mockClear();
...
`,
  ...
  it('prints the sign-up page and GitHub Issues links when not signed in', async () => {
    ...
    harness.auth.status.mockResolvedValueOnce({
      providers: [{ providerName: 'managed:nighthawk', hasToken: false }],
    });
  ...
  it('removes debug timing status from undone turns', async () => {
    const { driver, session } = await makeDriver();
    const previousDebug = process.env['NIGHTHAWK_DEBUG'];
    process.env['NIGHTHAWK_DEBUG'] = '1';
    try {
      driver.handleUserInput('hello');
      driver.sessionEventHandler.handleEvent(
        {
      ...
      if (previousDebug === undefined) {
        delete process.env['NIGHTHAWK_DEBUG'];
      } else {
        process.env['NIGHTHAWK_DEBUG'] = previousDebug;
      }
    }
  });
  ...
  it('deletes a pasted video’s daemon upload when the consuming turn ends', async () => {
    ...
    // carries a bare `nighthawk-file://` reference — no local cache copy.
    ...
    expect(parts?.[1]).toEqual({ type: 'video_url', videoUrl: { url: 'nighthawk-file://file-v1' } });
  ...
  it('queues a pasted video (nighthawk-file part) while a turn is streaming', async () => {
    const session = makeSession();
    const { driver } = await makeDriver(session);
    const imageStore = (driver as unknown as { imageStore: ImageAttachmentStore }).imageStore;
    const attachment = imageStore.addVideo('video/mp4', '/tmp/clip.mp4');
  ...
  it('falls back to retained bytes when a queued image upload expires before dispatch', async () => {
    ...
    driver.sendQueuedMessage(session, {
      ...
      parts: [
        { type: 'image_url', imageUrl: { url: 'nighthawk-file://file-expired' } },
      ],
  ...
  it('keeps an image staging upload across lazy session creation (v2 engine)', async () => {
    ...
    await vi.waitFor(() => {
      expect(session.prompt).toHaveBeenCalledWith(
        [{ type: 'image_url', imageUrl: { url: 'nighthawk-file://file-lazy' } }],
        { promptId: expect.any(String) },
      );
    });
    expect(harness.deleteFile).not.toHaveBeenCalled();
  ...
  it('waits briefly for a pending paste ingestion so the submit uses the daemon-ref form', async () => {
    ...
    await vi.waitFor(() => {
      expect(session.prompt).toHaveBeenCalledWith(
        [
          { type: 'text', text: 'describe ' },
          { type: 'image_url', imageUrl: { url: 'nighthawk-file://file-late' } },
        ],
        { promptId: expect.any(String) },
      );
    });
  ...
  it('keeps staged media when a queued message is recalled into the editor', async () => {
    ...
    expect(requeued.parts).toContainEqual({
      type: 'image_url',
      imageUrl: { url: 'nighthawk-file://file-recall' },
    });
  ...
  it('shows an error instead of throwing when skill media materialization fails', async () => {
    ...
    const missing = imageStore.addVideo('video/quicktime', '/tmp/nighthawk-missing-source.mov');
  ...
  it('shows an error instead of throwing when plugin command media materialization fails', async () => {
    ...
    const { driver } = await makeDriver(session);
    const imageStore = (driver as unknown as { imageStore: ImageAttachmentStore }).imageStore;
    const missing = imageStore.addVideo('video/mp4', '/tmp/nighthawk-missing-source.mp4');
  ...
  it('shows the login prompt for auth.login_required session errors', async () => {
    ...
    const transcript = stripSgr(renderTranscript(driver));
    expect(transcript).toContain('Provider session expired. Send /connect to reconnect.');
  ...
  it('appends the /export-debug-zip hint beneath session error messages', async () => {
    ...
    expect(transcript).not.toContain('nighthawk export');
  });
  ...
  it('shows the spawned model on the subagent card at spawn, mapped through the model catalog', async () => {
    ...
    driver.state.appState.availableModels = {
      'k2-cheap': {
        provider: 'managed:nighthawk',
        model: 'nighthawk-k2-cheap',
        maxContextSize: 100_000,
        displayName: 'NightHawk K2 Cheap',
        capabilities: [],
      },
    };
    ...
    expect(stripSgr(renderTranscript(driver))).toContain('NightHawk K2 Cheap');
  });
  ...
  it('renders /status using the active session runtime status', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain('>_ NightHawk');
  ...
  it('toggles plugin MCP servers from the text command', async () => {
    ...
    driver.handleUserInput('/plugins mcp enable nighthawk-datasource data');

    await vi.waitFor(() => {
      expect(session.setPluginMcpServerEnabled).toHaveBeenCalledWith(
        'nighthawk-datasource',
        'data',
        true,
      );
    });
  ...
  it('installs from a positional source on /plugins install after trusting it', async () => {
    ...
    driver.handleUserInput('/plugins install ./plugins/nighthawk-datasource');
    ...
    await vi.waitFor(() => {
      expect(session.installPlugin).toHaveBeenCalledWith(
        resolve('/tmp/proj-a', './plugins/nighthawk-datasource'),
      );
    });
  });
  ...
  it('does not show the quota note for a same-id fork installed from a local path', async () => {
    ...
    driver.handleUserInput('/plugins install ./plugins/nighthawk-datasource-fork');
    ...
    await vi.waitFor(() => {
      const transcript = stripSgr(renderTranscript(driver));
      expect(transcript).toContain('Installed NightHawk Datasource');
    });
    expect(stripSgr(renderTranscript(driver))).not.toContain(
  ...
  it('loads a local plugin marketplace file and installs from it', async () => {
    ...
    process.env['NIGHTHAWK_PLUGIN_MARKETPLACE_URL'] = marketplacePath;
    const session = makeSession();
    const { driver } = await makeDriver(session);
    ...
    await vi.waitFor(() => {
      expect(stripSgr(panel.render(120).join('\n'))).toContain('NightHawk Datasource');
    });
    // The pinned NightHawk WebBridge row leads the Official tab, so move down to
    // the NightHawk Datasource entry before installing.
    panel.handleInput('\u001B[B');
    panel.handleInput('\r');
  ...
  it('returns to the plugin list when a marketplace install fails', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(rendered).toContain('NightHawk Datasource');
  ...
  it('removes a plugin record without auto-running any cleanup skill', async () => {
    ...
    driver.handleUserInput('/plugins remove nighthawk-webbridge');
    ...
    await vi.waitFor(() => {
      expect(session.removePlugin).toHaveBeenCalledWith('nighthawk-webbridge');
    });
    expect(session.activateSkill).not.toHaveBeenCalled();
  });
      ...
      plugins: [
        {
          ...
          displayName: 'NightHawk Datasource',
          description: 'Datasource plugin',
          source: './official/nighthawk-datasource.zip',
        },
      ],
    }))));
    const session = makeSession();
  ...
  it('shows an inline Official error when the marketplace is unreachable, keeping the panel open', async () => {
    const originalFetch = globalThis.fetch;
    process.env['NIGHTHAWK_PLUGIN_MARKETPLACE_URL'] = 'https://example.test/marketplace.json';
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => {
  ...
  it('toggles plugin MCP servers from the overview MCP picker', async () => {
    ...
    const session = makeSession({
      ...
      getPluginInfo: vi.fn(async () => ({
        ...
        hasErrors: false,
        source: 'local-path',
        root: '/plugins/nighthawk-datasource',
        manifest: undefined,
        mcpServers: [
          {
            name: 'metadata',
            runtimeName: 'plugin-nighthawk-datasource-metadata',
            enabled: serverEnabled.get('metadata') === true,
            transport: 'stdio',
            command: 'node',
            args: ['./bin/nighthawk-datasource.mjs', 'metadata'],
          },
          {
            name: 'data',
            runtimeName: 'plugin-nighthawk-datasource-data',
            enabled: serverEnabled.get('data') === true,
            transport: 'stdio',
            command: 'node',
            args: ['./bin/nighthawk-datasource.mjs', 'data'],
          },
        ],
        diagnostics: [],
      })),
    ...
    expect(stripSgr(renderTranscript(driver))).not.toContain(
      'Disabled MCP server data for nighthawk-datasource. Run /reload or /new to apply.',
    );
  });
  ...
  it('applies /model selection with inline thinking state', async () => {
    ...
    expect(pickerOutput).toMatch(/NightHawk K2\s+NightHawk ← current/);
    expect(pickerOutput).toMatch(/❯ NightHawk Turbo\s+NightHawk/);
    ...
    expect(filteredOutput).toContain('NightHawk Turbo');
    expect(filteredOutput).not.toContain('NightHawk K2');
  ...
  it('uses the effective effort returned after a model-switch fallback', async () => {
    ...
    expect(driver.state.appState.model).toBe('turbo');
    expect(driver.state.appState.thinkingEffort).toBe('mid');
    expect(renderTranscript(driver)).toContain('Switched to nighthawk-turbo with thinking mid.');
  });
  ...
  it('does not write config when re-confirming the current effort in the picker', async () => {
    ...
    await vi.waitFor(() => {
      expect(renderTranscript(driver)).toContain('Already using NightHawk K2 with thinking high.');
    });
    expect(setConfig).not.toHaveBeenCalled();
    expect(session.setThinking).not.toHaveBeenCalled();
  });
  ...
  it('refreshes only OAuth provider models before opening /model picker', async () => {
      ...
      getConfig: vi.fn(async () => ({
        models: {
          k2: {
            ...
            model: 'nighthawk-k2',
            maxContextSize: 100,
            displayName: 'Old NightHawk K2',
            capabilities: ['thinking'],
          },
        },
      })),
    });
    const tui = driver as unknown as NighthawkTUI;
      ...
      tui.setAppState({
        availableModels: {
          k2: {
            ...
            model: 'nighthawk-k2',
            maxContextSize: 100,
            displayName: 'Fresh NightHawk K2',
            capabilities: ['thinking'],
          },
        },
      });
      return { changed: [], unchanged: ['managed:nighthawk'], failed: [] };
    });
    (
    ...
    await vi.waitFor(() => {
      ...
      expect(output).toContain('Fresh NightHawk K2');
      expect(output).not.toContain('Old NightHawk K2');
    });
  ...
  it('opens /model picker after 2s when OAuth refresh is still pending', async () => {
    ...
    try {
      ...
      expect(picker).toBeInstanceOf(TabbedModelSelectorComponent);
      const output = stripSgr((picker as TabbedModelSelectorComponent).render(120).join('\n'));
      expect(output).toContain('NightHawk K2');
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it('enables search in the shared model selector helper', async () => {
    ...
    const selection = runModelSelector(driver as any, {
      alpha: {
        provider: 'managed:nighthawk',
        model: 'nighthawk-alpha',
        maxContextSize: 100,
        displayName: 'NightHawk Alpha',
        capabilities: ['thinking'],
      },
      turbo: {
        provider: 'managed:nighthawk',
    ...
    expect(output).toContain('NightHawk Turbo');
    expect(output).not.toContain('NightHawk Alpha');
  ...
  it('updates terminal title through pi-tui without changing process title', async () => {
    ...
    try {
      process.title = 'nighthawk-test-runner';
      driver.sessionEventHandler.handleEvent(
        {
      ...
      expect(process.title).toBe('nighthawk-test-runner');
    } finally {
  ...
  it('forks the active session and stays in the source session', async () => {
    ...
    try {
      ...
      expect(copyTextToClipboard).toHaveBeenCalledWith(
        "cd '/tmp/proj-a' && nighthawk --resume 'ses-fork'",
      );
      const transcript = driver.state.transcriptContainer.render(120).join('\n');
      expect(transcript).toContain(
        "To enter the fork in a new process, run: cd '/tmp/proj-a' && nighthawk --resume 'ses-fork'",
      );
  ...
  it('prints a pushd-based fork resume command on Windows', async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        expect(copyTextToClipboard).toHaveBeenCalledWith(
          'pushd "D:\\proj" && nighthawk --resume "ses-fork"',
        );
      });
      expect(driver.getCurrentSessionId()).toBe('ses-source');
    } finally {
...
describe('/effort support_efforts override', () => {
  it('warns and applies efforts hidden by an Anthropic support_efforts override', async () => {
    ...
    const { driver } = await makeDriver(session, {
      getConfig: vi.fn(async () => ({
        providers: {
          compatible: { type: 'nighthawk', apiKey: 'test-key' },
        },
        models: {
          k2: {
            provider: 'compatible',
  ...
  it('offers no fallback efforts for an unknown model on a NightHawk provider using the Anthropic protocol', async () => {
    const { driver } = await makeDriver(makeSession(), {
      getConfig: vi.fn(async () => ({
        providers: {
          compatible: { type: 'nighthawk', apiKey: 'test-key' },
  ...
  it('keeps rejecting efforts hidden by a NightHawk support_efforts override', async () => {
    const session = makeSession();
    const { driver } = await makeDriver(session, {
      getConfig: vi.fn(async () => ({
        providers: {
          nighthawk: { type: 'nighthawk', apiKey: 'test-key' },
        },
        models: {
          k2: {
            provider: 'nighthawk',
            model: 'nighthawk-model',
            maxContextSize: 100,
            capabilities: ['thinking'],
