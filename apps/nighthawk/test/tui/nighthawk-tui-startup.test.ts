...
import type { MigrationPlan } from '@nighthawk/migration-legacy';
...
function makeStartupInput(
  cliOptions: Partial<NighthawkTUIStartupInput['cliOptions']> = {},
  tuiConfig: Partial<NighthawkTUIStartupInput['tuiConfig']> = {},
): NighthawkTUIStartupInput {
  return {
    cliOptions: {
      session: undefined,
      continue: false,
...
function makeDriver(harness: ReturnType<typeof makeHarness>, input: NighthawkTUIStartupInput) {
  const driver = new NighthawkTUI(harness as never, input) as unknown as StartupDriver;
...
describe('NighthawkTUI startup', () => {
  ...
  it('starts session-less on the v2 engine and carries startup flags to appState', async () => {
    const harness = makeHarness(makeSession(), {
      getConfig: vi.fn(async () => ({
        models: {
          k2: { model: 'nighthawk-v1', maxContextSize: 200 },
        },
        defaultModel: 'k2',
  ...
  it('mounts the docked fullscreen layout when NIGHTHAWK_TUI_FULL_SCREEN=1', async () => {
    const harness = makeHarness(makeSession());
    vi.stubEnv('NIGHTHAWK_TUI_FULL_SCREEN', '1');
  ...
  it('passes the CLI model override when creating a fresh startup session', async () => {
    ...
    const driver = makeDriver(harness, makeStartupInput({ model: 'nighthawk/k2.5' }));

    await expect(driver.init()).resolves.toBe(false);

    expect(harness.createSession).toHaveBeenCalledWith({
      workDir: '/tmp/proj-a',
      model: 'nighthawk/k2.5',
  ...
  it('applies the CLI model override when resuming a startup session', async () => {
    ...
    const driver = makeDriver(
      harness,
      makeStartupInput({ continue: true, model: 'nighthawk/k2.5' }),
    );

    await expect(driver.init()).resolves.toBe(true);

    expect(session.setModel).toHaveBeenCalledWith('nighthawk/k2.5');
    expect(driver.state.appState.model).toBe('nighthawk/k2.5');
  });
  ...
  it('does not resume a session from a different cwd and shows a cd hint', async () => {
    ...
    expect(driver.state.activeDialog).toBeNull();
    const expectedResumeCmd = `cd ${quoteShellArg('/tmp/proj-b')} && nighthawk --resume ${quoteShellArg('ses-other-cwd')}`;
  ...
  it('copies a shell-safe resume command for another cwd with metacharacters', async () => {
    ...
    expect(resumeSession).not.toHaveBeenCalled();
    const expectedResumeCmd = `cd ${quoteShellArg('/tmp/proj$(touch /tmp/pwned)')} && nighthawk --resume ${quoteShellArg('ses-other-cwd')}`;
    expect(copyTextToClipboardMock).toHaveBeenCalledWith(expectedResumeCmd);
    const transcript = driver.state.transcriptContainer.render(160).join('\n');
    expect(transcript).toContain(`To resume, run: ${expectedResumeCmd}`);
  });
  ...
  it('disposes terminal focus/theme tracking on the nighthawk migrate exit', async () => {
    const harness = makeHarness();
    const driver = makeDriver(harness, {
      ...makeStartupInput(),
    ...
    // `nighthawk migrate` exits via process.exit; startEventLoop() installed focus
  ...
  it('writes display state after rendering a once banner', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'nighthawk-startup-banner-'));
    process.env['NIGHTHAWK_HOME'] = dir;
  ...
  it('resumes a startup session when Windows workdir uses backslashes', async () => {
    const session = makeSession({ id: 'ses-target' });
    const harness = makeHarness(session, {
      listSessions: vi.fn(async () => [{ id: 'ses-target', workDir: 'C:/Users/nighthawk/project' }]),
    });
    const driver = makeDriver(harness, {
      ...makeStartupInput({ session: 'ses-target' }),
      workDir: String.raw`C:\Users\nighthawk\project`,
    });
