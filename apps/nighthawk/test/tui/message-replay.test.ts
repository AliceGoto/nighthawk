...
describe('NighthawkTUI resume message replay', () => {
  ...
  it('renders an uploaded image daemon ref as a bare placeholder on replay', async () => {
    // An uploaded image persists as a self-contained `nighthawk-file://` part; on
    ...
    const driver = await replayIntoDriver([
      message(
        ...
        [
          ...
          {
            type: 'image_url',
            imageUrl: { url: 'nighthawk-file://f_1?path=%2FUsers%2Falice%2Fmedia%2Ff_1.png' },
          },
        ],
        { origin: { kind: 'user' } },
      ),
    ...
    expect(transcript).not.toContain('nighthawk-file');
  });
