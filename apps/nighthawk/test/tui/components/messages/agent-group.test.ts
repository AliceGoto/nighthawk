import type { TUI } from '@nighthawk/pi-tui';
...
describe('AgentGroupComponent', () => {
  ...
  it('shows the bound model in the row stats once reported', () => {
    ...
    running.updateSubagentMetrics({ modelDisplay: 'K2.5 Coding' });
    // Non-phase updates are throttled; flush the pending refresh.
    vi.runOnlyPendingTimers();
    expect(renderText(group)).toContain('explore · inspect project · K2.5 Coding · 0 tools');
