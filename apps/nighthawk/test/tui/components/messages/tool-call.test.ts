import { visibleWidth, type TUI } from '@nighthawk/pi-tui';
...
describe('ToolCallComponent', () => {
  ...
  it('truncates a long file path from the head so the filename stays visible', () => {
    const longPath =
      'apps/nighthawk/src/tui/components/messages/tool-renderers/long-path/example/final-file.ts';
    const component = new ToolCallComponent(
      {
        id: 'call_long_path',
        name: 'Read',
    ...
    expect(out).not.toContain('apps/nighthawk/src/tui/components/messages/tool-renderers/long-pa…');
  });
  ...
  it('shows Read paths relative to the active workspace', () => {
    const component = new ToolCallComponent(
      {
        ...
        name: 'Read',
        args: { path: '/tmp/proj-a/apps/nighthawk/src/main.ts' },
      },
      {
    ...
    const expectedReadPath =
      process.platform === 'win32' ? 'apps\\nighthawk\\src\\main.ts' : 'apps/nighthawk/src/main.ts';
  ...
  it('renders a single foreground subagent without the generic Agent tool header', () => {
    ...
    component.appendSubToolCall({
      ...
      name: 'Read',
      args: { path: 'apps/nighthawk/src/tui/utils/background-agent-status.ts' },
    });
  ...
  it('shows the bound model in the subagent header and group snapshot once reported', () => {
    ...
    expect(out).not.toContain('K2.5 Coding');

    component.updateSubagentMetrics({ modelDisplay: 'K2.5 Coding' });

    out = strip(component.render(120).join('\n'));
    expect(out).toContain('Explore Agent Queued (explore project) · K2.5 Coding · 0 tools');
    expect(component.getSubagentSnapshot().model).toBe('K2.5 Coding');
  });
