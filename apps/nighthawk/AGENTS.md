# apps/nighthawk Development Guide

This file only contains rules local to `apps/nighthawk`. For cross-repo rules, see the root `AGENTS.md`.
...
`apps/nighthawk` is the terminal UI / CLI app. The entry chain is:
...
- `src/tui/nighthawk-tui.ts`: the `NighthawkTUI` coordinator — wires state, layout, editor, session, SDK events, and dialogs together, and dispatches slash-command handlers. Heavy logic is delegated to `controllers/`, not accumulated here.
...
- `NighthawkTUI` coordinates; it does not accumulate complex business rules. New logic that can be tested independently should be split into `controllers`, `commands`, `components`, `reverse-rpc`, or `utils` first.
- `controllers` own the heavy, independently-testable slices (event routing, streaming render, session replay, tasks browser, editor keyboard, auth). Event-routing and rendering logic belong here, not on the `NighthawkTUI` class.
- `commands` only owns slash-command declaration, parsing, and the parsed-result types. The actual execution can be dispatched from `NighthawkTUI`, but complex logic should continue to sink downward.
...
- Functions with no state / UI side effects do not belong as private methods on the `NighthawkTUI` class; put them in external utils.
